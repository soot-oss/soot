 ...
 * 
 * @author Timothy Hoffman, Marc Miltenberger
 */
public class SharedInitializationLocalSplitterTest {
  ...
  public void testSingleInitUsedAsBoolAndInt() {
    ...
    final SootMethod method = new SootMethod("testMethod", Collections.singletonList(tyBool), VoidType.v(), Modifier.PUBLIC | Modifier.STATIC);
    ...
    for (Unit unit : body.getUnits()) {
      InvokeExpr inv = ((Stmt)unit).getInvokeExprUnsafe();
      if (inv != null && inv.getMethod().getName().equals("println")) {
        assertTrue(inv.getArgs().get(0) instanceof Constant);
      }
    }

    //run type assigner
    PackManager.v().getTransform("jb.tr").apply(body);
    body.validate();
  }
}