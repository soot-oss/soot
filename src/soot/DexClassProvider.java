...
public class DexClassProvider implements ClassProvider {
			...
			if (!allDexFiles.isEmpty()) {// path is directory containing dex files or a single dex file
				...
				if (file.isFile()) {
					if (file.getName().endsWith(".apk") || file.getName().endsWith(".jar")
						...
						// check if the archive contains dex files and record the names if there are
						// multiple
						Set<String> entryNames = new HashSet<String>();
						ZipFile archive = null;
						try {
							archive = new ZipFile(file);
							for (Enumeration<? extends ZipEntry> entries = archive.entries(); entries
									.hasMoreElements();) {
	...
	public static Set<String> classesOfDex(File file, String dexName) throws IOException {
		...
		DexBackedDexFile d = dexName != null ? DexFileFactory.loadDexEntry(file, dexName, true, Opcodes.forApi(api))
