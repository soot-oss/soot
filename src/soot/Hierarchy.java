...
public class Hierarchy
{
  ...
  public void toDotFile() {
    ...
    Iterator classesIt = allclasses.iterator();
    while (classesIt.hasNext()){
      SootClass cls = (SootClass)classesIt.next();
      if (cls.isInterface()) {
	Iterator subinterfacesIt = getDirectSubinterfacesOf(cls).iterator();
	while (subinterfacesIt.hasNext()){
	  SootClass subinterface = (SootClass)subinterfacesIt.next();
	  DotGraphEdge edge = canvas.drawEdge(cls.getName(), 
					      subinterface.getName());
	  edge.setStyle(DotGraphConstants.EDGE_STYLE_DOTTED);
	}

	Iterator implementersIt = getDirectImplementersOf(cls).iterator();
	while (implementersIt.hasNext()) {
	  SootClass implementer = (SootClass)implementersIt.next();
	  DotGraphEdge edge = canvas.drawEdge(cls.getName(),
					      implementer.getName());
	  edge.setStyle(DotGraphConstants.EDGE_STYLE_DOTTED);
	}
      } else {
	Iterator subclassesIt = getDirectSubclassesOf(cls).iterator();
	while (subclassesIt.hasNext()) {
	  SootClass subclass = (SootClass)subclassesIt.next();
	  canvas.drawEdge(cls.getName(), subclass.getName());
	}
      }
    }
