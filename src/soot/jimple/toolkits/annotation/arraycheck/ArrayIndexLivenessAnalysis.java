...
class ArrayIndexLivenessAnalysis extends BackwardFlowAnalysis
{
	...
	if (debug) 
	    System.out.println("Enter ArrayIndexLivenessAnalysis");
	...
	if (debug) 
	    System.out.println("Leave ArrayIndexLivenessAnalysis");
    }
	...
	while (unitIt.hasNext())
	{
	    ...
	    if (stmt instanceof DefinitionStmt)
	    {
		DefinitionStmt asstmt = (DefinitionStmt)stmt;
		/* kill left hand side */
		...
		{
		    if (killarrayrelated)
			killArrayRelated.put(stmt, lhs);

		    if (killallarrayref)
			killAllArrayRef.put(stmt, new Boolean(true));
		}
	    }
	    else
	...
	while (tailIt.hasNext())
	{
	    ...
	    unitToAfterFlow.put(tail, new HashSet());
	}
    }
    */
