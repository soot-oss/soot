...
public final class OnFlyCallGraphBuilder {
	...
	private static final PrimType[] LONG_NARROWINGS = new PrimType[] { LongType.v(), IntType.v(), CharType.v(),
			ShortType.v(), ByteType.v(), ShortType.v() };
	private static final ByteType[] BYTE_NARROWINGS = new ByteType[] { ByteType.v() };
	private static final PrimType[] FLOAT_NARROWINGS = new PrimType[] { FloatType.v(), LongType.v(), IntType.v(),
			CharType.v(), ShortType.v(), ByteType.v(), ShortType.v(), };
			...
			if (mustBeNull || (ics.nullnessCode() == InvokeCallSite.MAY_BE_NULL
				...
				for (Type bType : s) {
					...
					if (bType instanceof NullType || bType instanceof ArrayType) {
						...
						SootClass baseClass = ((RefType) bType).getSootClass();
						Iterator<SootMethod> mIt = getPublicMethodIterator(baseClass, reachingTypes, methodSizes,
								mustNotBeNull);
						while (mIt.hasNext()) {
							SootMethod sm = mIt.next();
							cm.addVirtualEdge(ics.container(), ics.stmt(), sm, Kind.REFL_INVOKE, null);
		...
		if (receiverToSites.get(receiver) != null) {
			for (Iterator<VirtualCallSite> siteIt = receiverToSites.get(receiver).iterator(); siteIt.hasNext();) {
				...
				while (targets.hasNext()) {
					...
					cm.addVirtualEdge(MethodContext.v(site.container(), srcContext), site.stmt(), target, site.kind(),
							typeContext);
				}
			}
		}
			...
			if (constant == null) {
				...
				if (!Scene.v().containsClass(constant)) {
					...
					for (SootMethod clinit : EntryPoints.v().clinitsOf(sootcls)) {
						cm.addStaticEdge(MethodContext.v(site.container(), srcContext), site.stmt(), clinit,
								Kind.CLINIT);
					}
				}
			}
		...
		for (final Unit u : b.getUnits()) {
			...
			if (s.containsInvokeExpr()) {
				...
				if (ie instanceof InstanceInvokeExpr) {
					...
					if (subSig == sigStart) {
					...
					} else if (subSig == sigExecutorExecute || subSig == sigHandlerPost
							|| subSig == sigHandlerPostAtFrontOfQueue || subSig == sigHandlerPostAtTime
							|| subSig == sigHandlerPostAtTimeWithToken || subSig == sigHandlerPostDelayed) {
						...
						addVirtualCallSite(true, s, m, receiver, iie, sigHandlerHandleMessage, Kind.HANDLER);
					}
