...
public class VirtualCalls {
	...
	public void resolve(Type t, Type declaredType, Type sigType, NumberedString subSig, SootMethod container,
		...
		if (t instanceof RefType) {
			 ...
			 * subtyping allows contra-variance for return types and co-variance
			 * for parameters when overriding a method, these cases are also
			 * considered here.
			 * 
			 * Example: Classes A, B (B sub type of A), interface I with method
			 * public A foo(B b); and a class C with method public B foo(A a) {
			 * ... }. The extended class hierarchy will contain C as possible
			 * implementation of I.
			 * 
			 * Since Java has no multiple inheritance call by signature
			 * resolution is only activated if the base is an interface.
			 */
