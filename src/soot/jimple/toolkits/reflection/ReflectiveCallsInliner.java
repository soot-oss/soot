...
public class ReflectiveCallsInliner extends SceneTransformer {
	...
	private static final List<String> fieldSets = Arrays.asList("set", "setBoolean", "setByte", "setChar", "setInt",
			"setLong", "setFloat", "setDouble", "setShort");

	private static final List<String> fieldGets = Arrays.asList("get", "getBoolean", "getByte", "getChar", "getInt",
			"getLong", "getFloat", "getDouble", "getShort");
	...
	protected void internalTransform(String phaseName, @SuppressWarnings("rawtypes") Map options) {
		if (!initialized) {
			...
			UNINTERPRETED_METHOD = Scene.v().makeMethodRef(
					Scene.v().getSootClass("soot.rtlib.tamiflex.OpaquePredicate"), "getFalse",
		...
		for (SootMethod m : RTI.methodsContainingReflectiveCalls()) {
			...
			}
			{
				...
				if (!constructorNewInstanceSignatures.isEmpty()) {
					inlineRelectiveCalls(m, constructorNewInstanceSignatures,
							ReflectionTraceInfo.Kind.ConstructorNewInstance);
					if (Options.v().validate())
						b.validate();
				}
			}
		...
		for (SootMethod m : RTI.methodsContainingReflectiveCalls()) {
			{
				if (!RTI.classForNameClassNames(m).isEmpty()) {
					SootFieldRef fieldRef = Scene.v().makeFieldRef(reflCallsClass, "classForName",
							RefType.v("java.util.Set"), true);
					...
					for (String className : RTI.classForNameClassNames(m)) {
						InterfaceInvokeExpr invokeExpr = Jimple.v().newInterfaceInvokeExpr(setLocal, addMethodRef,
								StringConstant.v(callSiteId + className));
			...
			}
			{
				if (!RTI.classNewInstanceClassNames(m).isEmpty()) {
					SootFieldRef fieldRef = Scene.v().makeFieldRef(reflCallsClass, "classNewInstance",
							RefType.v("java.util.Set"), true);
					Local setLocal = localGen.generateLocal(RefType.v("java.util.Set"));
					newUnits.add(Jimple.v().newAssignStmt(setLocal, Jimple.v().newStaticFieldRef(fieldRef)));
			...
			}
			{
				if (!RTI.constructorNewInstanceSignatures(m).isEmpty()) {
					SootFieldRef fieldRef = Scene.v().makeFieldRef(reflCallsClass, "constructorNewInstance",
							RefType.v("java.util.Set"), true);
					Local setLocal = localGen.generateLocal(RefType.v("java.util.Set"));
					newUnits.add(Jimple.v().newAssignStmt(setLocal, Jimple.v().newStaticFieldRef(fieldRef)));
					for (String constrSig : RTI.constructorNewInstanceSignatures(m)) {
						InterfaceInvokeExpr invokeExpr = Jimple.v().newInterfaceInvokeExpr(setLocal, addMethodRef,
								StringConstant.v(callSiteId + constrSig));
						newUnits.add(Jimple.v().newInvokeStmt(invokeExpr));
					}
					callSiteId++;
				}
			}
			{
				if (!RTI.methodInvokeSignatures(m).isEmpty()) {
					SootFieldRef fieldRef = Scene.v().makeFieldRef(reflCallsClass, "methodInvoke",
							RefType.v("java.util.Set"), true);
					Local setLocal = localGen.generateLocal(RefType.v("java.util.Set"));
					newUnits.add(Jimple.v().newAssignStmt(setLocal, Jimple.v().newStaticFieldRef(fieldRef)));
					for (String methodSig : RTI.methodInvokeSignatures(m)) {
						InterfaceInvokeExpr invokeExpr = Jimple.v().newInterfaceInvokeExpr(setLocal, addMethodRef,
								StringConstant.v(callSiteId + methodSig));
						newUnits.add(Jimple.v().newInvokeStmt(invokeExpr));
					}
					callSiteId++;
				}
	...
	private void inlineRelectiveCalls(SootMethod m, Set<String> targets, Kind callKind) {
		...
		while (iter.hasNext()) {
			...
			if (s.containsInvokeExpr()) {
				...
				if (callKind == Kind.ClassForName && (ie.getMethodRef().getSignature()
									...
									.newInvokeStmt(
											Jimple.v()
													.newStaticInvokeExpr(
															Scene.v()
																	.getMethod(
																			"<soot.rtlib.tamiflex.ReflectiveCalls: void knownClassForName(int,java.lang.String)>")
																	.makeRef(),
															IntConstant.v(callSiteId), classNameValue)));
				} else if (callKind == Kind.ClassNewInstance && ie.getMethodRef().getSignature()
									...
									.newInvokeStmt(
											Jimple.v()
													.newStaticInvokeExpr(
															Scene.v()
																	.getMethod(
																			"<soot.rtlib.tamiflex.ReflectiveCalls: void knownClassNewInstance(int,java.lang.Class)>")
																	.makeRef(),
				...
				} else if (callKind == Kind.MethodInvoke && ie.getMethodRef().getSignature().equals(
											...
											Jimple.v()
													.newStaticInvokeExpr(
															Scene.v()
																	...
																	.makeRef(),
															IntConstant.v(callSiteId), recv, methodLocal)));
	...
	private SootMethod createNewMethod(Kind callKind, String target, Type fieldSetGetType) {
		...
		case ClassNewInstance: {
			...
			SpecialInvokeExpr constrCallExpr = Jimple.v().newSpecialInvokeExpr(freshLocal, Scene.v().makeMethodRef(
					targetClass, SootMethod.constructorName, Collections.<Type>emptyList(), VoidType.v(), false));
		...
		case ConstructorNewInstance: {
			...
			SootMethod constructor = Scene.v().getMethod(target);
			SpecialInvokeExpr constrCallExpr = Jimple.v().newSpecialInvokeExpr(freshLocal, constructor.makeRef(),
					Arrays.asList(paramLocals));
			InvokeStmt constrCallStmt2 = Jimple.v().newInvokeStmt(constrCallExpr);
			newUnits.add(constrCallStmt2);
			// add: retLocal = freshLocal
			newUnits.add(Jimple.v().newAssignStmt(retLocal, freshLocal));
		...
		case MethodInvoke: {
			...
			if (method.isStatic())
				...
				invokeExpr = Jimple.v().newVirtualInvokeExpr(freshLocal, method.makeRef(),
						Arrays.<Value>asList(paramLocals));
		...
		case FieldSet: {
			...
			newUnits.insertBeforeNoRedirect(
					Jimple.v().newIdentityStmt(value, Jimple.v().newParameterRef(fieldSetGetType, 1)), replStmt);
	...
	private void insertCastOrUnboxingCode(Local lhs, Local rhs, Chain<Unit> newUnits) {
		...
		if (lhs.getType() instanceof PrimType) {
			if ((rhs.getType() instanceof PrimType)) {
				...
				SootMethodRef ref = Scene.v().makeMethodRef(boxedType.getSootClass(),
						lhs.getType().toString() + "Value", Collections.<Type>emptyList(), lhs.getType(), false);
