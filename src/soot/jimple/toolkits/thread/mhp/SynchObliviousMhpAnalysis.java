...
//		logger.info("    MHP: AllocNodesFinder");
...
//		logger.info("    MHP: StartJoinFinder");
		...
		// Build MHP Lists
//		logger.info("    MHP: Building MHP Lists");
...
//		List mainMethods = new ArrayList();
		...
		if(optionPrintDebug)
			logger.info(mainThread.toString());
		...
		while(addedNew)
		{
			...
			while(it.hasNext())
			{
				...
				if(mayHappenInParallelInternal(someStartMethod, someStartMethod))
				{
					...
					it.remove();
					if(optionPrintDebug)
						logger.info(someThread.toString());
					addedNew = true;
				}
			}
		}
    ...
    public boolean mayHappenInParallel(SootMethod m1, Unit u1, SootMethod m2, Unit u2)
    {
   		if(optionThreaded)
		{
			...
			// Wait until finished
			logger.info("[mhp] waiting for analysis thread to finish");
			try
			{
				self.join();
			}
    ...
    { 
   		if(optionThreaded)
		{
			...
			// Wait until finished
			logger.info("[mhp] waiting for thread to finish");
			try
			{
				self.join();
			}
