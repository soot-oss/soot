...
public class LockAllocationBodyTransformer extends BodyTransformer {
	...
	protected void internalTransform(Body b, FlowSet fs, List<CriticalSectionGroup> groups,
									...
									// Scene.v().getMainClass())
		{
			...
			if (addingNewClinit) {
				clinitMethod = Scene.v().makeSootMethod("<clinit>", new ArrayList(), VoidType.v(),
			...
			for (int i = 1; i < groups.size(); i++) {
				...
				if (!tnGroup.useDynamicLock && !tnGroup.useLocksets) {
					...
					Stmt initStmt = Jimple.v().newInvokeStmt(
							Jimple.v().newSpecialInvokeExpr(lockObj[i], initMethod.makeRef(), Collections.EMPTY_LIST));
					if (addingNewClinit)
					...
					// other fns)
					Stmt assignStmt = Jimple.v().newAssignStmt(Jimple.v().newStaticFieldRef(globalLockObj[i].makeRef()),
							lockObj[i]);
					if (addingNewClinit)
					...
					if (lock instanceof Ref) {
						if (lock instanceof InstanceFieldRef) {
							...
							if (ifr.getBase() instanceof FakeJimpleLocal)
								lock = reconstruct(b, units, ifr,
										(tn.entermonitor != null ? tn.entermonitor : tn.beginning),
					...
					if (csr.exceptionalEnd != null) {
						...
						if (csr.end != null) {
							...
							for (Pair earlyEnd : csr.earlyEnds) {
								...
								if (lastEnd == null || (units.contains(lastEnd) && units.contains(end)
										&& units.follows(end, lastEnd)))
									lastEnd = end;
							}
						}
						...
						if (lastEnd == null)
							throw new RuntimeException(
									"Lock Region has no ends!  Where should we put the exception handling???");

						// Add throwable
						Local throwableLocal = Jimple.v().newLocal("throwableLocal" + (throwableNum++),
			...
			// deal with waits and notifys
			{
				for (Unit uNotify : tn.notifys) {
					...
					Stmt newNotify = Jimple.v()
							.newInvokeStmt(Jimple.v().newVirtualInvokeExpr(clo, sNotify.getInvokeExpr().getMethodRef()
									.declaringClass().getMethod("void notifyAll()").makeRef(), Collections.EMPTY_LIST));
					if (newPrep != null) {
						Stmt tmp = (Stmt) newPrep.clone();
	...
	public InstanceFieldRef reconstruct(Body b, PatchingChain<Unit> units, InstanceFieldRef lock, Stmt insertBefore,
		...
		if (!(fakeBase.getInfo() instanceof LockableReferenceAnalysis))
			throw new RuntimeException(
					"InstanceFieldRef cannot be reconstructed due to missing LocksetAnalysis info: " + lock);
		...
		if (base instanceof InstanceFieldRef) {
			...
			throw new RuntimeException(
					"InstanceFieldRef cannot be reconstructed because it's base is of an unsupported type"
							+ base.getType() + ": " + base);
	...
	static public Value getLockFor(EquivalentValue lockEqVal) {
		...
		if (lock instanceof StaticFieldRef || lock instanceof NewStaticLock) {
			...
			SootMethod initMethod = objectClass.getMethod("void <init>()");
			Stmt initStmt = Jimple.v().newInvokeStmt(
					Jimple.v().newSpecialInvokeExpr(lockLocal, initMethod.makeRef(), Collections.EMPTY_LIST));
			if (addingNewClinit)
				clinitUnits.add(initStmt);
			else
				clinitUnits.insertBeforeNoRedirect(initStmt, firstStmt);
