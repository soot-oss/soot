...
//		optionIncludeEmptyPossibleEdges = PhaseOptions.getBoolean( options, "include-empty-edges" ); // not a real option yet
		...
		{
	    	logger.info("[wjtp.tn] *** Build May-Happen-in-Parallel Info *** " + (new Date()));
    	...
    	if(optionDoTLO)
    	{
	    	logger.info("[wjtp.tn] *** Find Thread-Local Objects *** " + (new Date()));
	    	if(mhp != null)
	    	...
	    	{
		    	tlo.precompute();
	    		logger.info("[wjtp.tn] TLO totals (#analyzed/#encountered): " + SmartMethodInfoFlowAnalysis.counter + "/" + ClassInfoFlowAnalysis.methodCount);
		    }
	    	else
	    		logger.info("[wjtp.tn] TLO so far (#analyzed/#encountered): " + SmartMethodInfoFlowAnalysis.counter + "/" + ClassInfoFlowAnalysis.methodCount);
    	}
		...
		Date start = new Date();
    	logger.info("[wjtp.tn] *** Find and Name Transactions *** " + start);
    	...
    	// nesting model.
    	logger.info("[wjtp.tn] *** Find Transitive Read/Write Sets *** " + (new Date()));
    	...
    	{
    		logger.info("[wjtp.tn] TLO totals (#analyzed/#encountered): " + SmartMethodInfoFlowAnalysis.counter + "/" + ClassInfoFlowAnalysis.methodCount);
			logger.info("[wjtp.tn] Time for stages utilizing on-fly TLO: " + time + "s");
    	}
...
//*/    	
    	...
    	logger.info("[wjtp.tn] *** Calculate Locking Groups *** " + (new Date()));
		...
		logger.info("[wjtp.tn] *** Detect the Possibility of Deadlock *** " + (new Date()));
    	...
    	logger.info("[wjtp.tn] *** Calculate Locking Objects *** " + (new Date()));
		...
		else // for locksets and dynamic locks
		{
			...
			if(optionUseLocksets)
			{
				for( CriticalSection tn : criticalSections )
				{
					if( tn.group != null )
					{
						logger.info("[wjtp.tn] " + tn.name + " lockset: " + locksetToLockNumString(tn.lockset, lockToLockNum) + (tn.group.useLocksets ? "" : " (placeholder)"));
					}
				}
			}
		}
		...
		{
			logger.info("[wjtp.tn] *** Detect " + (optionAvoidDeadlock ? "and Correct " : "") + "the Possibility of Deadlock for Locksets *** " + (new Date()));
			...
			logger.info("[wjtp.tn] *** Reorder Locksets to Avoid Deadlock *** " + (new Date()));
			dd.reorderLocksets(lockToLockNum, (HashMutableEdgeLabelledDirectedGraph) deadlockGraph);
		}
		
		// *** Print Output and Transform Program ***
    	logger.info("[wjtp.tn] *** Print Output and Transform Program *** " + (new Date()));
			...
			if(tn.group.useDynamicLock || tn.group.useLocksets) // if attempting to use a dynamic lock or locksets
			{
				...
				if(optionUseLocksets)
				{
					...
					if(tn.lockset == null || tn.lockset.size() <= 0)
					{
						...
						logger.info("[wjtp.tn] Lock: num " + lockNum + " type " + newStaticLock.getType() + " obj " + newStaticLock);
						...
						for( EquivalentValue lockEqVal : tn.lockset )
						{
							...
							if( lockPT != null )
							{
								...
								for(int i = 0; i < lockPTSets.size(); i++)
								{
									...
									{
										logger.info("[wjtp.tn] Lock: num " + i + " type " + lock.getType() + " obj " + lock);
								...
								if(!foundLock)
								{
									logger.info("[wjtp.tn] Lock: num " + lockPTSets.size() + " type " + lock.getType() + " obj " + lock);
							...
							else // static field locks and pathological cases...
							{
								...
								if( lockToLockNum.get(lockEqVal) != null )
								{
									...
									logger.info("[wjtp.tn] Lock: num " + lockNum + " type " + lock.getType() + " obj " + lock);
	...
	public void printGraph(Collection<CriticalSection> AllTransactions, CriticalSectionInterferenceGraph ig, Map<Value, Integer> lockToLockNum)
	{
		...
		logger.info("[transaction-graph]" + (optionUseLocksets ? "" : " strict") + " graph transactions {"); // "\n[transaction-graph] start=1;");
...
//								objString = lockObject[group].toString();
					...
					if(Scene.v().getReachableMethods().contains(tn.method))
						logger.info("[transaction-graph] " + tn.name + " [name=\"" + tn.method.toString() + "\" style=\"setlinewidth(3)\"];");
					else
						logger.info("[transaction-graph] " + tn.name + " [name=\"" + tn.method.toString() + "\" color=cadetblue1 style=\"setlinewidth(1)\"];");
								...
								if(!visited.contains(tn2) && ig.mayHappenInParallel(tn, tn2))
								{
									for(EquivalentValue lock2EqVal : tn2.lockset)
									{
										...
										if(lockNum.intValue() == lock2Num.intValue())
										{
											...
											logger.info("[transaction-graph] " + tn.name + " -- " + tn2.name + " [color=" + color + " style=" + (lockNum >= 0 ? "dashed" : "solid") + " exactsize=1 style=\"setlinewidth(3)\"];");
										}
									}
								}
							}
					...
					else
					{
						...
						while(tnedgeit.hasNext())
						{
							...
							if(tnedge.setNumber == group + 1)
								logger.info("[transaction-graph] " + tn.name + " -- " + tnedge.name + " [color=" + (edge.size > 0 ? "black" : "cadetblue1") + " style=" + (tn.setNumber > 0 && tn.group.useDynamicLock ? "dashed" : "solid") + " exactsize=" + edge.size + " style=\"setlinewidth(3)\"];");
						}
					}
				}
			...
			while(tnIt.hasNext())
			{
				...
				if(tn.setNumber == -1)
				{
					if(!printedHeading)
					{
						...
						logger.info("[transaction-graph] subgraph lone {\n[transaction-graph] rank=source;");
						printedHeading = true;
					}
					if(Scene.v().getReachableMethods().contains(tn.method))
						logger.info("[transaction-graph] " + tn.name + " [name=\"" + tn.method.toString() + "\" style=\"setlinewidth(3)\"];");
					...
					while(tnedgeit.hasNext())
					{
						...
						if(tnedge.setNumber != tn.setNumber || tnedge.setNumber == -1)
							logger.info("[transaction-graph] " + tn.name + " -- " + tnedge.name + " [color=" + (edge.size > 0 ? "black" : "cadetblue1") + " style=" + (tn.setNumber > 0 && tn.group.useDynamicLock ? "dashed" : "solid") + " exactsize=" + edge.size + " style=\"setlinewidth(1)\"];");
					}
				}
			}
			if(printedHeading)
	...
	public void printTable(Collection<CriticalSection> AllTransactions, MhpTester mhp)
	{
		...
		while(tnIt7.hasNext())
		{
			...
			logger.info("[transaction-table] Transaction " + tn.name + (reachable ? " reachable" : " dead") + (mhpself ? " [called from >= 2 threads]" : " [called from <= 1 thread]"));
			logger.info("[transaction-table] Where: " + tn.method.getDeclaringClass().toString() + ":" + tn.method.toString() + ":  ");
			logger.info("[transaction-table] Orig : " + tn.origLock);
			logger.info("[transaction-table] Prep : " + tn.prepStmt);
			logger.info("[transaction-table] Begin: " + tn.entermonitor);
			G.v().out.print("[transaction-table] End  : early:" + tn.earlyEnds.toString() + " exc:" + tn.exceptionalEnd + " through:" + tn.end + " \n");
			logger.info("[transaction-table] Size : " + tn.units.size());
			if(tn.read.size() < 100)
			...
			logger.info("[transaction-table] Group: " + tn.setNumber + "\n[transaction-table] ");
		}
	}
