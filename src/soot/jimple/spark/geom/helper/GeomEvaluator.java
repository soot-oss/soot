 ...
 * constraints evaluation graph size, etc; 2. Virtual function resolution
 * comparison; 3. Static casts checking; 4. All pairs alias analysis; 5.
 * Building heap graph.
 * 
 * @author xiao
 * 
...
public class GeomEvaluator {
	...
	public void reportBasicMetrics() 
	{
		...
		Histogram pts_size_bar_geom = new Histogram(limits), 
				pts_size_bar_spark = new Histogram(limits);
		
		Histogram type_size_bar_geom = new Histogram(limits), 
				type_size_bar_spark = new Histogram(limits);
		...
		for (IVarAbstraction pn : ptsProvider.pointers) {
			...
			for (AllocNode obj : obj_set) {
				if (obj.getType() instanceof AnySubType) {
					SootClass rc = ((AnySubType) obj.getType()).getBase()
							.getSootClass();
					List<SootClass> list = null;
					if (rc.isInterface())
						list = Scene.v().getActiveHierarchy()
								.getImplementersOf(rc);
					else
						list = Scene.v().getActiveHierarchy()
								.getSubclassesOfIncluding(rc);
			...
			var.getP2Set().forall(new P2SetVisitor() {
				public final void visit(Node n) {
					...
					if (an.getType() instanceof AnySubType) {
						SootClass rc = ((AnySubType) an.getType()).getBase()
								.getSootClass();
						List<SootClass> list = null;
						if (rc.isInterface())
							list = Scene.v().getActiveHierarchy()
		...
		outputer.printf(
				...
				(double) total_geom_ins_pts / (n_legal_var),
				(double) total_spark_pts / n_legal_var);
		outputer.printf(
				"Total/Average Context Sensitive Points-to Tuples [core pointers]: %d / %.3f \n",
				total_geom_sen_pts, (double) total_geom_sen_pts / (n_legal_var));
		outputer.println("The largest points-to set size [core pointers]: "
				+ max_pts_geom + " (" + max_pts_spark + ")");

		outputer.println();
		pts_size_bar_geom.printResult(ptsProvider.ps,
				"Points-to Set Sizes Distribution [core pointers]:",
				pts_size_bar_spark);
		type_size_bar_geom.printResult(ptsProvider.ps,
				"Points-to Set Types Distribution [core pointers]:",
				type_size_bar_spark);
	}
	...
	{
		int n_callsites = 0, n_user_callsites = 0;
	...
	/**
	 * Count how many alias pairs in each function that are made up by all the
	 * variables accessed by that function.
	 */
	public void checkAliasAnalysis() 
	{
		...
		for (Node v : access_expr) {
			if ( ptsProvider.isExceptionPointer(v) )
				continue;

			pn = ptsProvider.findInternalNode(v);
			pn = pn.getRepresentative();
			if (pn.hasPTResult()) al.add(pn);
		}
