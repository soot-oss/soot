 ...
 *
 * <p>
 * <code>ThrowableSet</code> does not implement the {@link java.util.Set}
 ...
 * <code>ThrowableSet</code>. The <code>ThrowableSet</code>s returned by
 * {@link #whichCatchableAs(RefType)} cannot be involved in subsequent
 * <code>add()</code> or <code>whichCatchableAs()</code> operations. That is,
 * given
 *
 * <blockquote> <code>p = s.whichCatchableAs(r)</code> </blockquote>
 *
 * for any <code>ThrowableSet</code> <code>s</code> and {@link soot.RefType
 * RefType} <code>r</code>, and
 *
 * <blockquote> <code>t == p.getUncaught()</code> or
 * <code>t == p.getCaught()</code> </blockquote>
 *
 * then calls to <code>t.add(r)</code>, <code>t.add(a)</code>, and
 * <code>s.add(t)</code>, will throw an
 * {@link ThrowableSet.AlreadyHasExclusionsException}, for any
 * <code>RefType</code> <code>r</code>, {@link AnySubType} <code>a</code>, and
 * <code>ThrowableSet</code> <code>t</code>.
 * </p>
 *
 * <p>
 * Actually the restrictions implemented are not quite so strict (there are some
 * combinations of <code>whichCatchableAs()</code> followed by
 * <code>add()</code> which will not raise an exception), but a more accurate
 ...
 * <code>ThrowableSet</code>'s anticipated uses: we expect
 * <code>ThrowableSet</code>s to grow by accumulating all the exception types
 * that a given {@link Unit} may throw, then, shrink as the types caught by
 * different exception handlers are removed to yield the sets representing
 * exceptions which escape those handlers.
 * </p>
 * 
 * <p>
 * The <code>ThrowableSet</code> class is intended to be immutable (hence the
 * <code>final</code> modifier on its declaration). It does not take the step of
 * guaranteeing immutability by cloning the <code>RefLikeType</code> objects it
 * contains, though, because we trust {@link Scene} to enforce the existence of
 * only one <code>RefLikeType</code> instance with a given name.
 * </p>
 */
...
public final class ThrowableSet {
	...
	public static class Manager {
		...
		/**
		 * <code>ThrowableSet</code> containing all the asynchronous and virtual
		 * machine errors, which may be thrown by any bytecode instruction at
		 * any point in the computation.
		 */
		final ThrowableSet VM_ERRORS;

		/**
		 * <code>ThrowableSet</code> containing all the exceptions that may be
		 * thrown in the course of resolving a reference to another class,
		 * including the process of loading, preparing, and verifying the
		 * referenced class.
		 */
		...
		/**
		 * <code>ThrowableSet</code> containing all the exceptions that may be
		 * thrown in the course of resolving a reference to a field.
		 */
		final ThrowableSet RESOLVE_FIELD_ERRORS;

		/**
		 * <code>ThrowableSet</code> containing all the exceptions that may be
		 * thrown in the course of resolving a reference to a non-static method.
		 */
		final ThrowableSet RESOLVE_METHOD_ERRORS;

		/**
		 * <code>ThrowableSet</code> containing all the exceptions which may be
		 * thrown by instructions that have the potential to cause a new class
		 * to be loaded and initialized (including UnsatisfiedLinkError, which
		 * is raised at runtime rather than linking type).
		 */
		...
		public Manager(Singletons.Global g) {
			...
			// Runtime errors:
			RUNTIME_EXCEPTION = Scene.v().getRefType(
			...
			vmErrorSet
					.add(Scene.v().getRefType("java.lang.StackOverflowError"));
			...
			if (!Options.v().j2me())
				resolveClassErrorSet.add(AnySubType.v(Scene.v().getRefType(
		 ...
		 * exceptions included in <code>include</code> minus the set of
		 * exceptions included in <code>exclude</code>. Creates a new
		 * <code>ThrowableSet</code> only if there was not already one whose
		 * contents correspond to <code>include</code> - <code>exclude</code>.
		 * </p>
		 *
		 * @param include
		 *            A set of {@link RefLikeType} objects representing
		 *            exception types included in the result; may be
		 *            <code>null</code> if there are no included types.
		 *
		 * @param exclude
		 *            A set of {@link AnySubType} objects representing exception
		 *            types excluded from the result; may be <code>null</code>
		...
		public String reportInstrumentation() {
			...
			StringBuffer buf = new StringBuffer("registeredSets: ")
					...
					.append(addsOfAnySubType).append("\naddsOfSet: ")
					...
					.append("\nremovesOfAnySubType: ")
					.append(removesOfAnySubType).append("\nremovesFromMap: ")
					.append(removesFromMap).append("\nremovesFromMemo: ")
					.append(removesFromMemo).append("\nremovesFromSearch: ")
					.append(removesFromSearch).append("\nregistrationCalls: ")
	...
	public static class AlreadyHasExclusionsException extends
	 ...
	 * to be excluded from the types represented by this
	 * <code>ThrowableSet</code>. To simplify the implementation, once a
	 * <code>ThrowableSet</code> has any excluded types, the various
	 ...
	 * <code>ThrowableSet</code>. If the mapping (k,v) is in
	 * <code>memoizedAdds</code> and k is a <code>ThrowableSet</code>, then v is
	 * the set that results from adding all elements in k to <code>this</code>.
	 * If (k,v) is in <code>memoizedAdds</code> and k is a {@link RefLikeType},
	 * then v is the set that results from adding k to <code>this</code>.
	 */
	...
	/**
	 * Constructs a <code>ThrowableSet</code> which contains the exception types
	 * represented in <code>include</code>, except for those which are also in
	 * <code>exclude</code>. The constructor is private to ensure that the only
	 * way to get a new <code>ThrowableSet</code> is by adding elements to or
	 * removing them from an existing set.
	 *
	 * @param include
	 *            The set of {@link RefType} and {@link AnySubType} objects
	 *            representing the types to be included in the set.
	 * @param exclude
	 ...
	 * <code>s</code> in addition to those in this <code>ThrowableSet</code>.
	 *
	 * @param s
				...
				if (newType instanceof RefType) {
					for (RefLikeType incumbentType : resultSet) {
						if (incumbentType instanceof RefType) {
							...
							RefType incumbentBase = ((AnySubType) incumbentType)
									.getBase();
					...
					for (Iterator<RefLikeType> j = resultSet.iterator(); j.hasNext();) {
						...
						if (incumbentType instanceof RefType) {
							...
							if (newBase == incumbentBase) {
								...
								j.remove();
								changes++;
							} else if (hierarchy.canStoreType(newBase,
									incumbentBase)) {
								// No need to add this class.
								addNewType = false;
							}
	 ...
	 *
	 * @return <code>true</code> if this set contains an exception type that
	 *         might be caught by <code>catcher</code>, false if it does not.
	 */
	...
	public Pair whichCatchableAs(RefType catcher) {
		...
		for (AnySubType exclusion : exceptionsExcluded) {
			...
			if (h.canStoreType(catcher, exclusionBase)) {
				...
				uncaughtExcluded = addExceptionToSet(exclusion,
						uncaughtExcluded);
			}
		}
		...
		for (RefLikeType inclusion : exceptionsIncluded) {
			if (inclusion instanceof RefType) {
				...
				if (catcher.getSootClass().isPhantom()) {
					...
					uncaughtIncluded = addExceptionToSet(inclusion,
							uncaughtIncluded);
					uncaughtExcluded = addExceptionToSet(AnySubType.v(catcher),
							uncaughtExcluded);
					caughtIncluded = addExceptionToSet(AnySubType.v(catcher),
							caughtIncluded);
		...
		ThrowableSet caughtSet = Manager.v().registerSetIfNew(caughtIncluded,
				caughtExcluded);
		ThrowableSet uncaughtSet = Manager.v().registerSetIfNew(
				uncaughtIncluded, uncaughtExcluded);
	...
	public static class Pair {
		 ...
		 * @param uncaught
		 *            The set of exceptions to be returned when
		 *            {@link #getUncaught()} is called on the constructed
		 *            <code>ThrowableSet.Pair</code>.
		 */
		 ...
		 * <code>ThrowableSet.Pair</code>s representing the same set of caught
		 * and uncaught exception types.
		 *
		 * @param o
		 *            the <code>Object</code> to compare to this
		 *            <code>ThrowableSet.Pair</code>.
		 *
	 ...
	 *
	 * @param set
	 *            The <code>Set</code> to which to add the types, or
	...
	/**
	 * Returns a cryptic identifier for this <code>ThrowableSet</code>, used to
	...
	/**
	 * Returns an {@link Iterator} over a {@link Collection} of Throwable types
	 * which iterates over its elements in a consistent order (maintaining an
	 * ordering that is consistent across different runs makes it easier to
	 * compare sets generated by different implementations of the CFG classes).
	 *
	 * @param coll
	...
	/**
	 * <p>
	 * Produce an abbreviated representation of this <code>ThrowableSet</code>,
	 * suitable for human consumption. The abbreviations include:
	 * </p>
	 * 
	 * <ul>
	 *
	 * <li>The strings &ldquo;<code>java.lang.</code>&rdquo; is stripped from
	 * the beginning of exception names.</li>
	 *
	 * <li>The string &ldquo;<code>Exception</code>&rdquo; is stripped from the
	 * ends of exception names.</li>
	 *
	 * <li>Instances of <code>AnySubType</code> are indicated by surrounding the
	 * base type name with parentheses, rather than with the string &ldquo;
	 * <code>Any_subtype_of_</code>&rdquo;</li>
	 *
	 * <li>If this <code>ThrowableSet</code> includes all the elements of
	 * {@link ThrowableSet.Manager#VM_ERRORS VM_ERRORS}, they are abbreviated as
	...
	/**
	 * A package-private method to provide unit tests with access to the
	 * {@link RefLikeType} objects which represent the <code>Throwable</code>
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ ((exceptionsExcluded == null) ? 0 : exceptionsExcluded
						.hashCode());
		result = prime
				* result
				+ ((exceptionsIncluded == null) ? 0 : exceptionsIncluded
						.hashCode());
		return result;
	}
