 ...
 * expressions.  Also provides a couple of convenience functions.
 *
...
public class SEvaluator
{
    ...
    public static boolean isValueConstantValued(Value op)
    {
        if(op instanceof PhiExpr) {
            Iterator argsIt = ((PhiExpr) op).getArgs().iterator();
    ...
    public static Value getConstantValueOf(Value op) 
    {
        ...
        return (Value) ((PhiExpr) op).getArgs().get(0);
    }
    ...
    {
        Iterator argsIt = op.getArgs().iterator();
        Constant firstConstant = null;
