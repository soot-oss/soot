...
public class CycleFinder implements FactFinder {
			...
			for (List<AugmentedStmt> cal : component_list) {
				...
				if (entry_points.size() > 1) {
					...
					for (AugmentedStmt au : entry_points)
						asgEntryPoints.addLast(asg
								.get_AugStmt(au.get_Stmt()));
				...
				wasg.calculate_Reachability(succ_stmt, new HashSet<AugmentedStmt>(),
						entry_point);
				IterableSet<AugmentedStmt> cycle_body = get_CycleBody(entry_point,
						succ_stmt, asg, wasg);
				...
				if (characterizing_stmt != null) {
					checkExceptionLoop: for (ExceptionNode en : body.get_ExceptionFacts()) {
						...
						if (tryBody.contains(asg
								.get_AugStmt(characterizing_stmt.get_Stmt()))) {
				...
				/*
				 * if (tryBody.contains( asg.get_AugStmt(
				 * characterizing_stmt.get_Stmt()))) {
				 * 
				 * if (checkExceptionNodes.contains( en) == false)
				 * checkExceptionNodes.add( en);
				 * 
				 * Iterator cbit = cycle_body.snapshotIterator(); while
				 * (cbit.hasNext()) { AugmentedStmt cbas = (AugmentedStmt)
				 * cbit.next();
				 * 
				 * if (tryBody.contains( cbas) == false) cycle_body.remove(
				 * cbas); } } }
				 * 
				 * enlit = checkExceptionNodes.iterator(); exceptionNestingLoop:
				 * while (enlit.hasNext()) { ExceptionNode en = (ExceptionNode)
				 * enlit.next();
				 * 
				 * Iterator cbit = cycle_body.iterator(); while (cbit.hasNext())
				 * { AugmentedStmt cbas = (AugmentedStmt) cbit.next();
				 * 
				 * if (en.get_TryBody().contains( cbas) == false) {
				 * characterizing_stmt = null; break exceptionNestingLoop; } } }
				 * }
				 */
				...
				if (characterizing_stmt == null) {
					...
					body.consume_Condition(asg.get_AugStmt(characterizing_stmt
							.get_Stmt()));
					wasg.remove_AugmentedStmt(characterizing_stmt);

					IfStmt condition = (IfStmt) characterizing_stmt.get_Stmt();
					if (cycle_body.contains(asg.get_AugStmt(condition
							.getTarget())) == false)
						condition
								.setCondition(ConditionFlipper
										.flip((ConditionExpr) condition
												.getCondition()));
	...
	private List<List<AugmentedStmt>> build_component_list(AugmentedStmtGraph asg) {
		...
		/*
		 * 26th Jan 2006 Nomair A. Naeem This could be potentially bad since
		 * self loops will also get removed Adding code to check for self loop
	...
	private AugmentedStmt find_CharacterizingStmt(AugmentedStmt entry_point,
			IterableSet<AugmentedStmt> sc_component, AugmentedStmtGraph asg) {
		/*
		...
		/*
		 * We're not a while loop. Get the candidates for condition on a
		 * do-while loop.
		 */
		...
		while (worklist.isEmpty() == false) {
			...
			for (AugmentedStmt so : worklist.removeFirst().csuccs) {
				...
				if ((sc_component.contains(so))
						&& (touchSet.contains(so) == false)) {
					worklist.addLast(so);
					touchSet.add(so);
				}
			}
		...
		throw new RuntimeException(
				"Somehow didn't find a condition for a do-while loop!");
	}

	private IterableSet<AugmentedStmt> get_CycleBody(AugmentedStmt entry_point,
			AugmentedStmt boundary_stmt, AugmentedStmtGraph asg,
	...
	private void fix_MultiEntryPoint(DavaBody body, AugmentedStmtGraph asg,
			LinkedList<AugmentedStmt> entry_points, IterableSet<AugmentedStmt> scc) {
		AugmentedStmt naturalEntryPoint = get_NaturalEntryPoint(entry_points,
				scc);
		 ...
		 * empty targets list to constructor of GTableSwitch since then it just
		 * creates an empty array to hold the targets.. we intend to fill these
		 * in later using the setTarget method
		 * 
		 * hence the hack is to just send in null fully aware that they are
		...
		TableSwitchStmt tss = new GTableSwitchStmt(controlLocal, 0,
		...
		while (epit.hasNext()) {
			...
			for (AugmentedStmt pas : entryPoint.cpreds) {
				if ((pas == indirectionStmt)
						|| ((entryPoint != naturalEntryPoint) && (scc
								.contains(pas))))
					continue;
				...
				AssignStmt asnStmt = new GAssignStmt(controlLocal,
						DIntConstant.v(count, null));
	...
	private AugmentedStmt get_NaturalEntryPoint(
			LinkedList<AugmentedStmt> entry_points, IterableSet<AugmentedStmt> scc) {
	...
	private void DFS(AugmentedStmt as, HashSet<AugmentedStmt> touchSet,
			HashSet<AugmentedStmt> backTargets, IterableSet<AugmentedStmt> scc) {
