...
public class SynchronizedBlockFinder implements FactFinder
{
    ...
    /*
      ...
      monitorLocalSet contains all the locals that are used in monitors
      monitorEnterSet contains all enterMonitorStmts Statements
    */
	...
	while (asgit.hasNext()) {
		...
		if (synchSet != null) {
		     ...
		     * synch body
		     * exit monitor r2
		     * we need to realize that r1 and r2 are the same
    ...
    /*
     * as2locals: contains all locals which have unknown level for this augmented stmt
	...
	while (sccit.hasNext()) {
	    ...
	    //add to as2color each augstmt belonging to the component as the key and the color white as the value
	...
	//viSeeds contains all the augmentedStmts with unknown level in the method (for all scc)
	...
	while (worklist.isEmpty() == false) {
		...
		while (mlsit.hasNext()) {
		    ...
		    //if the level for a local is set to unknown and the level for the same local is not
	...
	while (asgit.hasNext()) {
		...
		if (local2level.get( local) == VARIABLE_INCR) {
		    ...
		    /*
		      Nomair A. Naeem  08-FEB-2005
		      BUG IN CODE. The getLast method returns a NoSuchElementException if list is empty.
    ...
    /*
     * Inputs:
     * as: the seed stmt, i.e. the first statement of a strongly connected component
     * component: the strongly connected stmt being processed
     * as2rml: a hashmap which contains an augmentedstmt as key and a hashmap with local to level mapping as value
	...
	while (sit.hasNext()) {
		...
		while (mlsit.hasNext()) {
		    ...
		    Value local = (Value) mlsit.next();
		    /*
		     * update the sucessors hashtable with level values from the seed stmt
		     * The loca2Level is the hashmap of local-->level for the seed
		     * The sLocal2Level is the hashmap of local --> level for the successor of the seed
		     */
    ...
    /*
     * Created a synch set for each enter monitor stmt.
	...
	while (worklist.isEmpty() == false) {
		...
		while (mlsit.hasNext()) {
			 ...
			 * as this local and the predLevel is greater than 0 decrease the pred level
			 */
			...
			/*
			 * if the stmt is a monitor enter (which is true for the initial worklist)
			 * and the local it enters is the same
			 * as this local and the predLevel is greater or equal to 0 increase the pred level
			 */
			if ((ems.getOp() == local) && (predLevel >= 0))
			    predLevel++;
		    }
		    ...
		    /*
		      ...
		      add the sucessors of the stmt to the worklist
		    */
    ...
    /*
     * If a stmt is removed from the synchBody then all other stmts
     * which are dominated by this stmt should also be removed from the synchBody
     */
	...
	//System.out.println("starting verification");
	{
	     ...
	     * to the stmt right after the "to be created synch block" See if this is the case and remove
	     * the stmt
	     */
	    ...
	    //to compute last stmt in a tryBody one should find the stmt whose sucessor is not in the tryBody
	    outer:
		...
		while(succIt.hasNext()){
			...
			//tempas has a sucessor which is not part of the trybody hence this is the last stmt
			break outer;
		    }
		}
	...
	*/	
	 ...
	 * predecessor is an enter monitor stmt NOT present in the synchBody
	 */
	...
	while (synchIt.hasNext()) {
		...
		if (synchBody.contains( pas) == false) {
		    //the as stmt has a predecessor which is not part of the synchBody
	...
	/*
	 * Now that we know the exception case we can go through the synchBody and remove
	 * all stmts whose predecessor does not belong to the synchBody
	 */
	...
	/*
	 * The two bodies match
	 * check if the exception thrown is of type "throwable" and that there is only one catchlist
	 * The reason for doing this is that synchronized blocks get converted to a try catch block
	...
	//at this point we know that the two bodies match and that the en exception is a throwable exception

	/*
	 * find the entry point of the catch boody
	 * The entry point of the catch body is the first stmt in the catchbody
	 * This can be found by finiding the stmt whose predecessor is not part of the catch body
	 */
	...
	/*
	 * To verify the catchbody one has to go through the catchbody ONE stmt at a time.
	 * This is NOT a good approach as it is using PATTERN MATCHING
	 *
	 * FIRST STEP
	 * Need to verify that the entrypoint stmt and any goto stmts following it all point to the entrypoint
	 */
	...
	*/
    ...
    /*
      DefinitionStmt s is the start of the area of protection
      The exception sucessors of as should directly or indirectly point to s
    */
    ...
    /*
     *Input:
     *      head: this is the monitor enter stmt
     *      synchSet: this contains all sucessors of head which have level greater or equal to the head
     */
