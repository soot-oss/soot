...
public class DavaFlowSet<T> extends AbstractFlowSet<T> {
	 ...
	 * encountered the current DavaFlowSet is stored in the break/continue list
	 * with the appropriate label for the target code. This is how explicit
	 * breaks and continues are handled by the analysis framework
	 */
	HashMap<Serializable, List<DavaFlowSet<T>>> breakList;
	HashMap<Serializable, List<DavaFlowSet<T>>> continueList;

	/**
	 * To handle implicit breaks and continues the following HashMaps store the
	 * DavaFlowSets as value with the key being the targeted piece of code (an
	 * ASTNode)
	 */
	...
	/**
	 * A private method used to add an element into a List if it is NOT a
	 * duplicate
	 */
	private List<DavaFlowSet<T>> addIfNotDuplicate(List<DavaFlowSet<T>> into,
			DavaFlowSet<T> addThis) {
	...
	/**
	 * Checks whether the input stmt is an implicit break/continue A abrupt stmt
	 * is implicit if the SETLabelNode is null or the label.toString results in
	 * null
	 */
	...
	public void addToImplicitBreaks(DAbruptStmt ab, DavaFlowSet<T> set) {
		...
		if (!ab.is_Break())
			throw new RuntimeException(
					"Tried to add continue statement in the break list in DavaFlowSet.addToImplicitBreaks");
	 ...
	 */
	private List<DavaFlowSet<T>> copyDavaFlowSetList(List<DavaFlowSet<T>> currentList,
	...
	private boolean compareHashMaps(HashMap<Serializable, List<DavaFlowSet<T>>> thisMap,
		...
		while (keys.hasNext()) {
			...
			if (!alreadyDone) {
				/*
				 * we have come across a label which was not done by the first
				 * hashmap meaning it was NOT in the first hashMap
				 */
				return false;
			}
		}
