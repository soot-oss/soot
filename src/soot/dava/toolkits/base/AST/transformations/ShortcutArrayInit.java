...
public class ShortcutArrayInit extends DepthFirstAdapter {
	...
	public void inASTStatementSequenceNode(ASTStatementSequenceNode node){
		...
		while(it.hasNext()){
			...
			//have the size have the type, tempIt is poised at the current def stmt
			ValueBox[] array = new ValueBox[ ((IntConstant)size).value];
			success=true;
			for(int i=0;i<((IntConstant)size).value;i++){
				//check these many next stmts they better all be array initializations
			...
			if(success){
				...
				if(possible){
					...
					//make sure to mark the local in the DVariableDeclarations so that its not printed
	...
	/*
	 * Check that s is a definition stmt which assigns to array leftOp
	 * and index location index
	 */
	...
	/*
	 * Maybe its a multi-D array then we need to look for a DAssignStmt
	...
	/*
	 * Check that one is a definition stmt which declares an array 
	 * Check that two is a definition of leftOp at index location 
	 */
	...
	//actually all that is needed is that this localnot be printed there! it might be a good
	//idea to keep the local stored there since other analyses use that as the list of all
