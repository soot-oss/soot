...
public class LoadStoreOptimizer extends BodyTransformer
{
    ...
    {   
        ...
        if(Options.v().verbose())
            logger.info("[" + body.getMethod().getName() + "] Performing LoadStore optimizations...");
...
class Instance {
    ...
    void go() {
        if(!mUnits.isEmpty()) {                    
            ...
            if(debug){logger.info("Calling optimizeLoadStore(1)\n");}
            optimizeLoadStores(); 
        
            if(PhaseOptions.getBoolean(gOptions, "inter") ) {
                if(debug){logger.info("Calling doInterBlockOptimizations");}
                doInterBlockOptimizations(); 
                             
                //computeLocalDefsAndLocalUsesInfo();          
                //propagateLoadsBackwards();         if(debug)     logger.info("pass 3");         
                //optimizeLoadStores();      if(debug)   logger.info("pass 4"); 
                //propagateLoadsForward();   if(debug)   logger.info("pass 5"); 
                //propagateBackwardsIndependentHunk(); if(debug)  logger.info("pass 6");                        
            }

            if(PhaseOptions.getBoolean(gOptions, "sl2") || PhaseOptions.getBoolean(gOptions, "sll2")  ) {        
                mPass2 = true;
                if(debug){logger.info("Calling optimizeLoadStore(2)");}
                optimizeLoadStores();   
            }
        }        
    }
    ...
    private  void buildUnitToBlockMap()
    {
        ...
        if(debug) {
            logger.info("Method " +  mBody.getMethod().getName()+ " Block Graph: ");
            logger.info(blockGraph+"");
        }
                            ...
                            case 1:
                                ...
                                //if(block.getIndexInMethod() < 1 ) { // <13
                                        if(test == SUCCESS || test == SPECIAL_SUCCESS){
                                            ...
                                            //delme[
                                            if(debug) { logger.info("Store/Load elimination occurred case1.");}
                                            //delme]
                                        ...
                                        if(result == SUCCESS){        
                                            ...
                                            if(res == MAKE_DUP) {                                        
                                                ...
                                                hasChanged = true;         hasChangedFlag = false;
                                                  ...
                                                  if(debug) { logger.info("stack unit is: " + stackUnit + " stack type is " + underType);}
        ...
        if(storePred != null) {
            if(((Inst)storePred).getOutCount() == 1){
                ...
                if(h != 0) {           
                    ...
                    while(currentUnit != null) {
                        ...
                        if(h<0){ // xxx could be more flexible here?
                            if(debug) { logger.info("xxx: negative");}
                            return FAILURE;
                        }
                ...
                for(;;) {
                    ...
                    while(it2.hasNext()) {
                        Unit nu = it2.next();
                        if(debug) { logger.info("xxxspecial;success pushing forward stuff.");}
                        
                        
                        if(!canMoveUnitOver(nu, uu)){
                            if(debug) { logger.info("xxx: cant move over faillure" + nu);}
                            return FAILURE;
                        }
                        if(debug) { logger.info("can move" + nu + " over " + uu);}
                    }
                }        
                ...
                while(unitToMove != from) {        
                    ...
                    if(debug) { logger.info("moving " + unitToMove);}
                ...
                if(debug) { logger.info("xxx1success pushing forward stuff.");}
    ...
    {                
        if(debug) { 
            logger.info("Trying: " + from + "/" + to +  " in block  " + block.getIndexInMethod()+":" );
            logger.info("context:" + (aContext == STORE_LOAD_ELIMINATION ? 
        ...
        if(debug) { 
            logger.info("nshv = " + stackHeight);
            logger.info("mshv = " + minStackHeightAttained);
        }
        ...
        while(hasChanged) {
            ...
            if(aContext == STORE_LOAD_LOAD_ELIMINATION) {
                ...
                if(stackHeight == 0 && minStackHeightAttained == 0){
                    if(debug) { 
                        logger.info("xxx: succ: -1, makedup ");
                    }
                    return MAKE_DUP;
                }
                else if(stackHeight == -1 && minStackHeightAttained == -1){
                    if(debug) { logger.info("xxx: succ: -1, makedup , -1");}        
                    return MAKE_DUP;
                 }
                else if(stackHeight == -2 && minStackHeightAttained == -2){if(debug) { logger.info("xxx: succ -1 , make dupx1 ");}
                return MAKE_DUP1_X1; }
                
                else  if (minStackHeightAttained < -2) {
                    if(debug) { logger.info("xxx: failled due: minStackHeightAttained < -2 ");}
                    return FAILURE;
                }                
            }
            ...
            if(aContext == STORE_LOAD_ELIMINATION) {                
                if(stackHeight == 0 && minStackHeightAttained == 0){
                    if(debug) { logger.info("xxx: success due: 0, SUCCESS ");}
                    return SUCCESS;
                }
                /* xxx broken data depensie problem.
                    ...
                    if(u instanceof FieldGetInst)
                        if(block.getPredOf(u) instanceof Dup1Inst) {
                            ...
                            block.remove(from);
                            block.insertBefore(from, to);
                            if(debug) { logger.info("xxx: success due to 1, SPECIAL_SUCCESS2");}
            ...
            while( u != to) {        
                ...
                if(((Inst)u).getNetCount() == 1) {
                    ...
                    if(u instanceof LoadInst || u instanceof PushInst || u instanceof NewInst || u instanceof StaticGetInst || u instanceof Dup1Inst) {
                        ...
                        if(debug) { logger.info("1003:(LoadStoreOptimizer@stackIndependent): found unknown unit w/ getNetCount == 1" + u);}
                    }
                }
        ...
        if(isCommutativeBinOp(block.getSuccOf(to))) {
            if(aContext == STORE_LOAD_ELIMINATION && stackHeight == 1 && minStackHeightAttained == 0) {
                if(debug) { logger.info("xxx: commutative ");}
                return SPECIAL_SUCCESS;
            }
    ...
    private  boolean tryToMoveUnit(Unit unitToMove, Block block, Unit from, Unit to, int flag) 
    {
        ...
        if(debug) {logger.info("[tryToMoveUnit]: trying to move:" + unitToMove);}
        if(unitToMove == null) 
            return false;                
        ...
        while( current != block.getHead()) {   // do not go past basic block limit
            ...
            if(h < 0 ){
                if(debug) { logger.info("1006:(LoadStoreOptimizer@stackIndependent): Stack went negative while trying to reorder code.");}
            ...
            if(h == 0 && reachedStore == true) {
                if(!isRequiredByFollowingUnits(unitToMove, to)) {
                    if(debug) { logger.info("10077:(LoadStoreOptimizer@stackIndependent): reordering bytecode move: " + unitToMove + "before: " + current);}
                    block.remove(unitToMove);
        ...
        if(reorderingOccurred) {
            if(debug) { logger.info("reordering occured");}
            return true;
        } else {
            if(debug) { logger.info("1008:(LoadStoreOptimizer@stackIndependent):failled to find a new slot for unit to move");}
            return false;
        }
    }
        ...
        while(hasChanged) {
            ...
            while(it.hasNext()) {
                ...
                if(u instanceof LoadInst) {
                    if(debug) { logger.info("inter trying: " + u);}
                                ...
                                if(uses.size() == 1){
                                    if(allSuccesorsOfAreThePredecessorsOf(defBlock, loadBlock)) {
                                        if(getDeltaStackHeightFromTo(defBlock.getSuccOf(storeUnit), defBlock.getTail()) == 0) {
                                            ...
                                            if(debug) { logger.info(defBlock.toString() + loadBlock.toString());}
                                            ...
                                            if(res) {
                                                ...
                                                hasChanged = true;
                                                if(debug) { logger.info("inter-block opti occurred " + storeUnit + " " + u);}
                                                if(debug) { logger.info(defBlock.toString() + loadBlock.toString());}
                                            }
                                        }
                                    }
                                    ...
                                    if(def0Succs.get(0) == loadBlock && def1Succs.get(0)== loadBlock) {                                         
                                        if(loadBlock.getPreds().size() == 2) {
                                            if( (def0 == defBlock0.getTail()|| 
                                                ...
                                                hasChanged = true;
                                                if(debug) { logger.info("inter-block opti2 occurred " + def0);}
                                            } else { if(debug) { logger.info("failed: inter1");}}
                                        } else { if(debug) { logger.info("failed: inter2");}}
                                    } else { if(debug) { logger.info("failed: inter3");}}                                        
                                } else { if(debug) { logger.info("failed: inter4");}}
                            }        else { if(debug) { logger.info("failed: inter5");}}                        
                        } else { if(debug) { logger.info("failed: inter6");}}
                    }                         
                }        
            }
        }
