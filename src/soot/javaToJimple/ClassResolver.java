...
public class ClassResolver {
	...
	private void createClassDecl(polyglot.ast.ClassDecl cDecl) {
		...
		if ((staticFieldInits != null) || (staticInitializerBlocks != null)) {
			...
			if (!sootClass.declaresMethod("<clinit>", new ArrayList(), soot.VoidType.v())) {
				clinitMethod = Scene.v().makeSootMethod("<clinit>", new ArrayList(), soot.VoidType.v(),
						soot.Modifier.STATIC, new ArrayList<SootClass>());
		...
		if (cDecl.type().isLocal()) {
			AnonLocalClassInfo info = InitialResolver.v().finalLocalInfo()
					.get(new polyglot.util.IdentityKey(cDecl.type()));
	...
	private ArrayList<SootField> addFinalLocals(polyglot.ast.ClassBody cBody, ArrayList<IdentityKey> finalLocalsAvail,
		...
		while (newsIt.hasNext()) {
			...
			if (InitialResolver.v().finalLocalInfo().containsKey(new polyglot.util.IdentityKey(tempNewType))) {
				AnonLocalClassInfo lInfo = InitialResolver.v().finalLocalInfo()
						.get(new polyglot.util.IdentityKey(tempNewType));
		...
		while (!Util.getSootType(superType).equals(soot.Scene.v().getSootClass("java.lang.Object").getType())) {
			if (InitialResolver.v().finalLocalInfo().containsKey(new polyglot.util.IdentityKey(superType))) {
				AnonLocalClassInfo lInfo = InitialResolver.v().finalLocalInfo()
						.get(new polyglot.util.IdentityKey(superType));
				Iterator<IdentityKey> it = lInfo.finalLocalsAvail().iterator();
				while (it.hasNext()) {
					polyglot.types.LocalInstance li2 = (polyglot.types.LocalInstance) it.next().object();
					if (!sootClass.declaresField("val$" + li2.name(), Util.getSootType(li2.type()))) {
	...
	/**
	 * creates the Jimple for an anon class - in the AST there is no class decl
	 * for anon classes - the revelant fields and methods are created
	 */
	private void createAnonClassDecl(polyglot.ast.New aNew) {
		...
		InitialResolver.v().getInnerClassInfoMap().put(sootClass,
				new InnerClassInfo(outerClass, "0", InnerClassInfo.ANON));
		sootClass.setOuterClass(outerClass);
		...
		if (((polyglot.types.ClassType) aNew.objectType().type()).flags().isInterface()) {
			...
			/*
			 * Iterator aIt = aNew.arguments().iterator(); while
			 * (aIt.hasNext()){ polyglot.types.Type pType =
			 * ((polyglot.ast.Expr)aIt.next()).type();
			 * params.add(Util.getSootType(pType)); }
			 */
			method = Scene.v().makeSootMethod("<init>", params, soot.VoidType.v());
		}
		...
		if (((polyglot.types.ClassType) aNew.objectType().type()).isNested()) {
			...
			src.isSubType(Util.isSubType(aNew.anonType().outer(),
					((polyglot.types.ClassType) aNew.objectType().type()).outer()));
		}
	...
	private soot.SootClass getSpecialInterfaceAnonClass(soot.SootClass addToClass) {
		...
		if ((InitialResolver.v().specialAnonMap() != null)
				&& (InitialResolver.v().specialAnonMap().containsKey(addToClass))) {
			...
			Util.addInnerClassTag(specialClass, specialClass.getName(), addToClass.getName(), null,
	...
	private void handleAssert(polyglot.ast.ClassBody cBody) {
		...
		// make meth
		soot.SootMethod sootMethod = Scene.v().makeSootMethod(methodName, paramTypes, methodRetType,
				soot.Modifier.STATIC);
		...
		if (!sootClass.declaresMethod(methodName, paramTypes, methodRetType)) {
			...
			((soot.javaToJimple.PolyglotMethodSource) sootClass.getMethod(methodName, paramTypes, methodRetType)
					.getSource()).hasAssert(true);
		}
	}
	...
	private void createMethodDecl(polyglot.ast.MethodDecl method) {
		...
		soot.SootMethod sootMethod = createSootMethod(name, method.flags(), method.returnType().type(), parameters,
				exceptions);
		...
		if ((fieldInits != null) || (initializerBlocks != null)) {
			...
			while (methodsIt.hasNext()) {
				...
				if (next.getName().equals("<init>")) {
					...
					soot.javaToJimple.PolyglotMethodSource src = (soot.javaToJimple.PolyglotMethodSource) next
							.getSource();
			...
			while (nestedDeclsIt.hasNext() && !found) {
				...
				if (type.isLocal() && !type.isAnonymous()) {
					...
					if (InitialResolver.v().getLocalClassMap().containsVal(simpleName)) {
						createClassDecl(((polyglot.ast.LocalClassDecl) InitialResolver.v().getLocalClassMap()
								.getKey(simpleName)).decl());
						found = true;
					}
				} else {
	...
	private void handleInnerClassTags(polyglot.ast.ClassBody classBody) {
		...
		if ((InitialResolver.v().getInnerClassInfoMap() != null)
			...
			Util.addInnerClassTag(sootClass, sootClass.getName(),
					...
					soot.Modifier.isInterface(tag.getOuterClass().getModifiers())
							? soot.Modifier.STATIC | soot.Modifier.PUBLIC : sootClass.getModifiers());
	...
	private void addConstValTag(polyglot.ast.FieldDecl field, soot.SootField sootField) {
		...
		if (field.fieldInstance().constantValue() instanceof Integer) {
			...
			sootField.addTag(new soot.tagkit.IntegerConstantValueTag(
					((Character) field.fieldInstance().constantValue()).charValue()));
		} else if (field.fieldInstance().constantValue() instanceof Short) {
			sootField.addTag(new soot.tagkit.IntegerConstantValueTag(
					((Short) field.fieldInstance().constantValue()).shortValue()));
		} else if (field.fieldInstance().constantValue() instanceof Byte) {
			sootField.addTag(new soot.tagkit.IntegerConstantValueTag(
					((Byte) field.fieldInstance().constantValue()).byteValue()));
			...
			sootField.addTag(
					new soot.tagkit.LongConstantValueTag(((Long) field.fieldInstance().constantValue()).longValue()));
		} else if (field.fieldInstance().constantValue() instanceof Double) {
			// System.out.println("const val:
			// "+field.fieldInstance().constantValue());
			sootField.addTag(new soot.tagkit.DoubleConstantValueTag(
					(long) ((Double) field.fieldInstance().constantValue()).doubleValue()));
			// System.out.println(((Double)field.fieldInstance().constantValue()).doubleValue());
			soot.tagkit.DoubleConstantValueTag tag = (soot.tagkit.DoubleConstantValueTag) sootField
					.getTag("DoubleConstantValueTag");
			// System.out.println("tag: "+tag);
		} else if (field.fieldInstance().constantValue() instanceof Float) {
			sootField.addTag(new soot.tagkit.FloatConstantValueTag(
					((Float) field.fieldInstance().constantValue()).floatValue()));
			...
			throw new RuntimeException("Expecting static final field to have a constant value! For field: " + field
					+ " of type: " + field.fieldInstance().constantValue().getClass());
		}
	}
