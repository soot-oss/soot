...
public class Util {
	...
	public static void addInnerClassTag(soot.SootClass sc, String innerName, String outerName, String simpleName,
			int access) {
	...
	public static void addLnPosTags(soot.tagkit.Host host, polyglot.util.Position pos) {
		if (pos != null) {
			if (soot.options.Options.v().keep_line_number()) {
				if (pos.file() != null) {
					host.addTag(new soot.tagkit.SourceLnNamePosTag(pos.file(), pos.line(), pos.endLine(), pos.column(),
							pos.endColumn()));
				} else {
					host.addTag(
							new soot.tagkit.SourceLnPosTag(pos.line(), pos.endLine(), pos.column(), pos.endColumn()));
				}
			}
		}
	}
	...
	public static soot.Local getThis(soot.Type sootType, soot.Body body, HashMap getThisMap, LocalGenerator lg) {
		...
		// here we need an exact type I think
		/*
		 * if (bodyHasLocal(body, sootType)){ soot.Local l =
		 * getLocalOfType(body, sootType); getThisMap.put(sootType, l); return
		 * l; }
		 */
	...
	private static boolean bodyHasLocal(soot.Body body, soot.Type type) {
		...
		return false;
		/*
		 * soot.FastHierarchy fh = InitialResolver.v().hierarchy(); Iterator it
		 * = body.getDefBoxes().iterator(); while (it.hasNext()){ soot.ValueBox
		 * vb = (soot.ValueBox)it.next(); if ((vb.getValue() instanceof
		 * soot.Local) && (fh.canStoreType(type,
		 * vb.getValue().getType()))){//(vb.getValue().getType().equals(type))){
		 * return true; } } return false;
		 */
	}

	public static soot.Local getThisGivenOuter(soot.Type sootType, HashMap getThisMap, soot.Body body,
			LocalGenerator lg, soot.Local t2) {
	...
	private static soot.SootMethod makeOuterThisAccessMethod(soot.SootClass classToInvoke) {
		...
		PrivateFieldAccMethodSource src = new PrivateFieldAccMethodSource(
				classToInvoke.getFieldByName("this$0").getType(), "this$0",
