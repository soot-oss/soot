 ...
 * recreated if the Scene changes.
 *
...
public class FastHierarchy {
	 ...
	 * key is in value.getInterfaces(). This is one of the three maps that hold
	 ...
	 * interface) and key is in value.getInterfaces(). This is one of the three
	 * maps that hold the inverse of the relationships given by the
	 ...
	 * being allowed to be interfaces. If we don't know any of the classes, we
	 * always return false
	 */
	 ...
	 * stored in a variable of type parent. If child is an interface that is not
	 * a subinterface of parent, this method will return false even though some
	 * objects implementing the child interface may also implement the parent
	 * interface.
	 */
						...
						if (isVisible(concreteType, m)) {
							if (method.isAbstract())
								throw new RuntimeException(
										"abstract dispatch resolved to abstract method!\nAbstract Type: " + abstractType
	...
	public SootMethod resolveSpecialDispatch(SpecialInvokeExpr ie, SootMethod container) {
		 ...
		 * See VM Spec, 2nd Edition, Chapter 6, in the definition of
		 * invokespecial.
		 */
	...
	/**
	 * Gets the direct subclasses of a given class. The class needs to be
	 * resolved at least at the HIERARCHY level.
	 * 
	 * @param c
	 *            the class
