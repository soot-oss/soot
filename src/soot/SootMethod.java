...
/**
 * Soot representation of a Java method. Can be declared to belong to a
 * SootClass. Does not contain the actual code, which belongs to a Body. The
...
public class SootMethod extends AbstractHost implements ClassMember, Numberable, MethodOrMethodContext {
	...
	/**
	 * Nomair A. Naeem , January 14th 2006 Need it for the decompiler to create
	 * a new SootMethod The SootMethod can be created fine but when one tries to
	 * create a SootMethodRef there is an error because there is no declaring
	 * class set. Dava cannot add the method to the class until after it has
	 * ended decompiling the remaining method (new method added is added in the
	 * PackManager) It would make sense to setDeclared to true within this
	 * method too. However later when the sootMethod is added it checks that the
	 * method is not set to declared (isDeclared).
	 */
	...
	/**
	 * Returns true when some <code>SootClass</code> object declares this
	 * <code>SootMethod</code> object.
	 */
	@Override
	...
	/**
	 * Returns true if this method is not phantom, abstract or native, i.e. this
	 ...
	 * returns that.
	 *
	 * If you called Scene.v().loadClassAndSupport() for a class yourself, it
	 * will not be an application class, so you cannot get retrieve its active
	 ...
	 * bytecode (eg. [Ljava/lang/Object instead of java.lang.Object[]).
	 */
	...
	/**
	 * Returns the Soot signature of this method. Used to refer to methods
	 * unambiguously.
	 */
	...
	/**
	 * Returns the Soot subsignature of this method. Used to refer to methods
	 * unambiguously.
	 */
