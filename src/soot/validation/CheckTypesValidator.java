...
public class CheckTypesValidator implements BodyValidator {
	...
	private static final Logger logger =LoggerFactory.getLogger(CheckTypesValidator.class);
		...
		while(it.hasNext()) {
			...
			DefinitionStmt astmt=(DefinitionStmt) stmt;
	                if( !(astmt.getRightOp() instanceof CaughtExceptionRef ) ) {
	                    ...
	                    checkCopy(stmt, body, exception, leftType,rightType,errorSuffix);
			...
			if(iexpr instanceof InstanceInvokeExpr) {
			    ...
			    checkCopy(stmt, body, exception, called.declaringClass().getType(),
				      iiexpr.getBase().getType(),
				      " in receiver of call"+errorSuffix);
			}

			if(called.parameterTypes().size() != iexpr.getArgCount())
				exception.add(new ValidationException(stmt, "Argument count does not match the signature of the called function", "Warning: Argument count doesn't match up with signature in call"+errorSuffix+" in "+body.getMethod()));
			else 
