...
public class FixUndefinedLocals extends BodyTransformer implements IJbcoTransform {
  ...
  private Unit findInitializerSpotFor(Value v, Unit u, UnitGraph ug,
    ...
    while (preds.size() == 1) {
      Unit p = (Unit) preds.get(0);
      //if (p instanceof IdentityInst)
      //  break;

      u = p;
      preds = ug.getPredsOf(u);
    }

    if (preds.size() <= 1)
      return u;

    ArrayList nodef = new ArrayList();
    Iterator pIt = preds.iterator();
    while (pIt.hasNext()) {
      Unit u1 = (Unit) pIt.next();
      if (!gd.getGuaranteedDefs(u1).contains(v)) {
        nodef.add(u1);
      }
    }

    if (nodef.size() == preds.size())
      return u;

    if (nodef.size() == 1)
      return findInitializerSpotFor(v, (Unit) nodef.get(0), ug, gd);
