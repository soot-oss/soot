...
public class PackManager {
	...
	private void init() {
		...
		// CFG Viewer
		/*
		 * addPack(p = new BodyPack("cfg")); { p.add(new Transform("cfg.output",
		 * CFGPrinter.v())); }
		 */
		...
		// Dummy Dava Phase
		/*
		 * Nomair A. Naeem 13th Feb 2006 Added so that Dava Options can be added
		 * as phase options rather than main soot options since they only make
		 * sense when decompiling The db phase options are added in
		 * soot_options.xml
		 */
	...
	private void preProcessDAVA() {
		if (Options.v().output_format() == Options.output_format_dava) {
			...
			if (!isSourceJavac) {
				/*
				 * It turns out that the exception attributes of a method i.e.
				 * those exceptions that a method can throw are only checked by
				 * the Java compiler and not the JVM
				 *
				 * Javac does place this information into the attributes but
				 * other compilers dont hence if the source is not javac then we
				 * have to do this fancy analysis to find all the potential
				 * exceptions that might get thrown
				 *
				 * BY DEFAULT the option javac of db is set to true so we assume
				 * that the source is javac
				 *
	...
	private void runBodyPacks(SootClass c) {
		...
		if (produceDava) {
			...
			/*
			 * January 13th, 2006 SuperFirstStmtHandler might have set
			 * SootMethodAddedByDava if it needs to create a new method.
			 */
