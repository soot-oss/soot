...
public class ClassHierarchy 
{
    ...
    public class TypeNode
    {
        ...
        private IntSet descendants = new IntSet();
        ...
        TypeNode(RefType type)
        {
            ...
            BClass bclass = classManager.getClass(type.className);
            if(bclass.hasSuperClass())
            {
                parents.set(getTypeNode(RefType.v(bclass.getSuperClass().getName())).id);
            }
            for(Iterator i = bclass.getInterfaces().iterator(); i.hasNext(); )
            {
                parents.set(getTypeNode(RefType.v(((BClass) i.next()).getName())).id);
            }
            
            int[] parentArray = parents.elements();
            for(int i = 0; i < parentArray.length; i++)
            {
                TypeNode parent = (TypeNode) typeNodeInstances.elementAt(parentArray[i]);
        ...
        TypeNode(ArrayType type)
        {
            ...
            if(type.baseType instanceof RefType)
            {
                ...
                BClass bclass = classManager.getClass(baseType.className);
                if(bclass.hasSuperClass())
                {
                    parents.set(getTypeNode(ArrayType.v(RefType.v(bclass.getSuperClass().getName()),
                        type.numDimensions)).id);
                }
                for(Iterator i = bclass.getInterfaces().iterator(); i.hasNext(); )
                {
                    parents.set(getTypeNode(ArrayType.v(RefType.v(((BClass) i.next()).getName()),
                        type.numDimensions)).id);
                }
        ...
        public IntSet getAncestors()
        {
            return (IntSet) ancestors.clone();
        }

        /** Returns the ids of the descendants of this type node. **/
        public IntSet getDescendants()
        {
            return (IntSet) descendants.clone();
        }
