...
class Inliner {
   ...
   CallSite getCorrectCallSite ( InvokeExpr inve, MethodNode methodnode ) {
   ...
   Set examineMethodsToFixCallSites ( Collection callgraph, Resolver res ) {
      ...
      if ( NOLIB )
      {
         ...
         while ( callit.hasNext() )
         {
            ...
            if ( ! ( tempMN.getMethod().getDeclaringClass().getName().startsWith ( "java." ) || tempMN.getMethod().getDeclaringClass().getName().startsWith ("sun.") || tempMN.getMethod().getDeclaringClass().getName().startsWith("sunw.") || tempMN.getMethod().getDeclaringClass().getName().equals("javax.") || tempMN.getMethod().getDeclaringClass().getName().startsWith("org.") || tempMN.getMethod().getDeclaringClass().getName().equals("com.") ) )
            {
...
/*      int beforesize = 0, aftersize = 0;
         ...
         try {
            ...
            while ( methit.hasNext() )
            {
               ...
               try {
                  ...
                  beforesize = beforesize + ( ( Integer ) origSizeHT.get ( changedmethod.getSignature() ) ).intValue();
                  aftersize = aftersize + changedjb.getStmtList().size();
                  ...
                  beforesize = beforesize + changedjb.getStmtList().size();
                  aftersize = aftersize + changedjb.getStmtList().size();
      ...
      // System.out.println("Code expansion factor in changed classes : "+( (double) (100*aftersize/beforesize) ) );
*/
   ...
   void insertNullCheck ( Value baseval, Local nulllocal, StmtList targetbody, Iterator target )
   {
   ...
   boolean satisfiesCriteria ( SootMethod m, InvokeExpr ie ) {
      try {
         if ( NOLIB )
         {
            ...
            || m.getDeclaringClass().getName().startsWith ( "org." ) || m.getDeclaringClass().getName().startsWith ( "com." ) )
            return false;
         }

         if ( ! ( classesToAnalyze == null ) )
         {
            if ( ! classesToAnalyze.contains(m.getDeclaringClass().getName()) )
            {
               return false;
            }
   ...
   boolean satisfiesExceptionsAccess ( SootMethod m ) {
   ...
   Constant getCorrectConstant ( Constant constant ) {
   ...
   ArrayRef getCorrectArrayRef ( ArrayRef arrayref ) {
   ...
   SootField getCorrectField ( SootField field ) {
   ...
   InstanceFieldRef getCorrectInstanceFieldRef ( InstanceFieldRef
   instancefieldref ) {
   ...
   StaticFieldRef getCorrectStaticFieldRef ( StaticFieldRef staticfieldref
   ) {
   ...
   Value getCorrectValue ( Value rvalue ) {
   ...
   ConditionExpr getCorrectConditionExpr ( ConditionExpr conditionexpr ) {
   ...
   Expr getCorrectBinopExpr ( BinopExpr binopexpr ) {
   ...
   Expr getCorrectCastExpr ( CastExpr castexpr ) {
   ...
   Expr getCorrectInstanceOfExpr ( InstanceOfExpr instanceofexpr ) {
   ...
   Expr getCorrectInvokeExpr ( InvokeExpr invokeexpr ) {
   ...
   Expr getCorrectUnopExpr ( UnopExpr unopexpr ) {
   ...
   NewArrayExpr getCorrectNewArrayExpr ( NewArrayExpr newarrayexpr ) {
   ...
   NewExpr getCorrectNewExpr ( NewExpr newexpr ) {
   ...
   NewMultiArrayExpr getCorrectNewMultiArrayExpr ( NewMultiArrayExpr
   newmultiarrayexpr ) {
   ...
   void InlineMethod ( StmtList inlinablemethod, StmtList targetmethod, Iterator target ) {
   ...
   int CountIdentityStmts ( StmtList list ) {
   ...
   void FixupMethod ( StmtList targetMethod, Iterator fixupIterator,
   ...
   boolean satisfiesInvokeSpecialSafety ( SootMethod method ) {
   ...
   boolean isStrictSuperClass ( SootClass sc1, SootClass sc2 ) {
   ...
   Integer getCorrectModifier ( int modifiers ) {
   ...
   int getChangedClassModifiers ( String s, int modifiers ) {
   ...
   int getChangedMethodModifiers ( String s, int modifiers ) {
      int changedmodifiers = modifiers;
      changedmodifiers = changedmodifiers & 0xFFFB;
      changedmodifiers = changedmodifiers & 0xFFFD;
      changedmodifiers = changedmodifiers & 0xFFFE;
   ...
   int getChangedFieldModifiers ( String s, int modifiers ) {
      int changedmodifiers = modifiers;
      changedmodifiers = changedmodifiers & 0xFFFB;
      changedmodifiers = changedmodifiers & 0xFFFD;
      changedmodifiers = changedmodifiers & 0xFFFE;
