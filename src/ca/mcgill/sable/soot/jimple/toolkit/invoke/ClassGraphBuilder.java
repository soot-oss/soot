...
class ClassGraphBuilder{
   ...
   *
   */
   ClassNode getNode( String className ){
      ClassNode classNode ;
   ...
   */
   ...
   Set getIncorrectlyJimplifiedClasses() {
      return incorrectlyjimplified;
   }


   void setClassGraphNumbers() {
      ...
      for ( int i = 0 ; i < htkeys.length ; i++ )
      {
         try {
            ...
            boolean isOrg = isLibraryNode("org.",name);
            boolean isJavax = isLibraryNode("javax.",name);
            boolean isCom = isLibraryNode("com.",name);
            boolean isBenchmark = false;
            if ( isJava || isSun || isSunw || isOrg || isJavax || isCom )
            libraryClasses++;
            else
            {
               benchmarkClasses++;
            ...
            while ( bc.hasSuperClass() )
            {
               ...
               isOrg = isLibraryNode("org.",name);
               isJavax = isLibraryNode("javax.",name);
               isCom = isLibraryNode("com.",name);
               if ( ! ( isJava || isSun || isSunw || isOrg || isJavax || isCom ) )
               {
                  benchdepth++;
                  // benchtotaldepth++; 
   ...
   static boolean isLibraryNode ( String s1, String s2 )
   {
      int len = s1.length();
   ...
   boolean implementsRunnable(SootClass bclass) {
   ...
   */
   ClassNode CreateNodeAndAddToHT( SootClass c ){
   ...
   */
   List getClassesDirectlyUsed(String Name){
   ...
   *
   */
   void addAllClassesUsedBy( String Name ){
   ...
   * and build links
   */
   void addSuperClassesOf( ClassNode cn ){
   ...
   */
   Set getAllSubClassesOf( ClassNode classNode ){
   ...
   *
   */
   void buildClassGraph( String className ){
   ...
   *
   */
   void buildClassAndInterfaceGraph( String className ){
   ...
   */
   void buildClassAndInterfaceGraph(){
   ...
   */
   ...
   ClassNode findLeastSuperClass( String cName1 , String cName2 ){
