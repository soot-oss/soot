...
public class JimpleBodyBuilder extends AbstractJimpleBodyBuilder {
	...
	public soot.jimple.JimpleBody createJimpleBody(polyglot.ast.Block block, List formals, soot.SootMethod sootMethod) {
		...
		if ((outerIndex != -1) && (sootMethod.getName().equals("<init>"))) {
			...
			if (this0Field != null) {
				...
				soot.jimple.ParameterRef paramRef = soot.jimple.Jimple.v().newParameterRef(outerClass.getType(),
						formalsCounter);
				paramRefCount++;
		...
		if (finalsList != null) {
			...
			while (finalsIt.hasNext()) {
				...
				soot.jimple.ParameterRef paramRef = soot.jimple.Jimple.v().newParameterRef(sf.getType(),
						formalsCounter);
				paramRefCount++;
				soot.jimple.Stmt stmt = soot.jimple.Jimple.v().newIdentityStmt(lg.generateLocal(sf.getType()),
						paramRef);
				body.getUnits().add(stmt);
				formalsCounter++;
			}
	...
	private void handleFieldInits(soot.SootMethod sootMethod) {
		...
		ArrayList<FieldDecl> fieldInits = ((soot.javaToJimple.PolyglotMethodSource) sootMethod.getSource())
				.getFieldInits();
		if (fieldInits != null) {
	...
	private void handleOuterClassThisInit(soot.SootMethod sootMethod) {
		...
		if (this0Field != null) {
			soot.jimple.FieldRef fieldRef = soot.jimple.Jimple.v().newInstanceFieldRef(specialThisLocal,
					this0Field.makeRef());
	...
	private void handleStaticFieldInits(soot.SootMethod sootMethod) {
		...
		ArrayList<FieldDecl> staticFieldInits = ((soot.javaToJimple.PolyglotMethodSource) sootMethod.getSource())
				.getStaticFieldInits();
	...
	private void handleInitializerBlocks(soot.SootMethod sootMethod) {
		ArrayList<Block> initializerBlocks = ((soot.javaToJimple.PolyglotMethodSource) sootMethod.getSource())
	...
	private void handleStaticInitializerBlocks(soot.SootMethod sootMethod) {
		ArrayList<Block> staticInitializerBlocks = ((soot.javaToJimple.PolyglotMethodSource) sootMethod.getSource())
	...
	private soot.Local getLocal(polyglot.types.LocalInstance li) {
		...
		if (localsMap.containsKey(new polyglot.util.IdentityKey(li))) {
			...
			return sootLocal;
		} else if (body.getMethod().getDeclaringClass().declaresField("val$" + li.name(),
				Util.getSootType(li.type()))) {
			soot.Local fieldLocal = generateLocal(li.type());
			soot.SootFieldRef field = soot.Scene.v().makeFieldRef(body.getMethod().getDeclaringClass(),
					"val$" + li.name(), Util.getSootType(li.type()), false);
		 ...
		 */
			...
			while (!fieldFound) {
				if (!currentClass.declaresFieldByName("this$0")) {
					throw new RuntimeException("Trying to get field val$" + li.name()
							+ " from some outer class but can't access the outer class of: " + currentClass.getName()
							+ "!" + " current class contains fields: " + currentClass.getFields());
				}
				soot.SootClass outerClass = ((soot.RefType) currentClass.getFieldByName("this$0").getType())
						.getSootClass();
	...
	private soot.SootMethod makeLiFieldAccessMethod(soot.SootClass classToInvoke, polyglot.types.LocalInstance li) {
		...
		soot.SootMethod meth = Scene.v().makeSootMethod(name, paramTypes, Util.getSootType(li.type()),
				soot.Modifier.STATIC);

		classToInvoke.addMethod(meth);
		PrivateFieldAccMethodSource src = new PrivateFieldAccMethodSource(Util.getSootType(li.type()),
				"val$" + li.name(), false, classToInvoke);
	...
	/**
	 * If Stmts Creation - only add line-number tags to if (the other stmts
	 * needing tags are created elsewhere
	 */
	/*
	 ...
	 * 
	 * // handle cond polyglot.ast.Expr condition = ifExpr.cond(); soot.Value
	 * sootCond = base().createExpr(condition);
	 * 
	 * // pop true false noops right away soot.jimple.Stmt tNoop =
	 * (soot.jimple.Stmt)trueNoop.pop(); soot.jimple.Stmt fNoop =
	 * (soot.jimple.Stmt)falseNoop.pop();
	 * 
	 * boolean needIf = needSootIf(sootCond); if (!(sootCond instanceof
	 * soot.jimple.ConditionExpr)) { sootCond =
	 * soot.jimple.Jimple.v().newEqExpr(sootCond, soot.jimple.IntConstant.v(0));
	 * } else { sootCond =
	 * reverseCondition((soot.jimple.ConditionExpr)sootCond); sootCond =
	 * handleDFLCond((soot.jimple.ConditionExpr)sootCond); }
	 * 
	 * // add if soot.jimple.Stmt noop1 = soot.jimple.Jimple.v().newNopStmt();
	 * 
	 * if (needIf) { soot.jimple.IfStmt ifStmt =
	 * soot.jimple.Jimple.v().newIfStmt(sootCond, noop1);
	 * body.getUnits().add(ifStmt); // add line and pos tags
	 ...
	 * 
	 * // add consequence polyglot.ast.Stmt consequence = ifExpr.consequent();
	 * createStmt(consequence);
	 * 
	 * soot.jimple.Stmt noop2 = null; if (ifExpr.alternative() != null){ noop2 =
	 * soot.jimple.Jimple.v().newNopStmt(); soot.jimple.Stmt goto1 =
	 * soot.jimple.Jimple.v().newGotoStmt(noop2); body.getUnits().add(goto1); }
	 * 
	 ...
	 * 
	 * 
	 * // handle alternative polyglot.ast.Stmt alternative =
	 * ifExpr.alternative(); if (alternative != null){ createStmt(alternative);
	...
	private void createBranchingExpr(polyglot.ast.Expr expr, soot.jimple.Stmt tgt, boolean boto) {
		if (expr instanceof polyglot.ast.Binary
				&& ((polyglot.ast.Binary) expr).operator() == polyglot.ast.Binary.COND_AND) {
			...
			if (boto) {
				...
				&& ((polyglot.ast.Unary) expr).operator() == polyglot.ast.Unary.NOT) {
	 ...
	 * body.getUnits().add(noop2);
	 * 
	 * // handle cond soot.jimple.Stmt continueStmt =
	 * (soot.jimple.Stmt)condControlNoop.pop();
	 * body.getUnits().add(continueStmt); condControlNoop.push(continueStmt);
	 * 
	 * polyglot.ast.Expr condition = whileStmt.cond(); soot.Value sootCond =
	 * base().createExpr(condition); soot.jimple.Stmt tNoop =
	 * (soot.jimple.Stmt)trueNoop.pop(); soot.jimple.Stmt fNoop =
	 * (soot.jimple.Stmt)falseNoop.pop(); boolean needIf = needSootIf(sootCond);
	 * if (!(sootCond instanceof soot.jimple.ConditionExpr)) { sootCond =
	 * soot.jimple.Jimple.v().newEqExpr(sootCond, soot.jimple.IntConstant.v(0));
	 * } else { sootCond =
	 * reverseCondition((soot.jimple.ConditionExpr)sootCond); sootCond =
	 * handleDFLCond((soot.jimple.ConditionExpr)sootCond); }
	 ...
	 * soot.jimple.Jimple.v().newIfStmt(sootCond, noop1);
	 * 
	 * body.getUnits().add(ifStmt); Util.addLnPosTags(ifStmt.getConditionBox(),
	 * condition.position()); Util.addLnPosTags(ifStmt, condition.position()); }
	 * 
	 * body.getUnits().add(tNoop); createStmt(whileStmt.body());
	 * soot.jimple.GotoStmt gotoLoop =
	 * soot.jimple.Jimple.v().newGotoStmt(noop2); body.getUnits().add(gotoLoop);
	 * 
	 * body.getUnits().add((soot.jimple.Stmt)(endControlNoop.pop()));
	 * body.getUnits().add(noop1); body.getUnits().add(fNoop);
	 * condControlNoop.pop(); }
	 */
	 ...
	 * 
	 * // create true/false noops to handle cond and/or soot.jimple.Stmt tNoop =
	 * soot.jimple.Jimple.v().newNopStmt(); soot.jimple.Stmt fNoop =
	 ...
	 * body.getUnits().add(continueStmt); condControlNoop.push(continueStmt);
	 * 
	 * // handle label continue //if ((labelContinueMap != null) &&
	 * (labelContinueMap.containsKey(lastLabel))){ if (labelMap != null &&
	 * labelMap.containsKey(doStmt)){
	 * body.getUnits().add((soot.jimple.Stmt)labelMap.get(doStmt)); } /*if
	 * ((labelContinueMap != null) && (labelStack != null) &&
	 * (!labelStack.isEmpty()) &&
	 * (labelContinueMap.containsKey(((LabelKey)labelStack.peek()).label()))){
	 * body.getUnits().add((soot.jimple.Stmt)labelContinueMap.get(((LabelKey)
	 * labelStack.peek()).label())); }
	 */

	/*
	 * trueNoop.push(tNoop); falseNoop.push(fNoop);
	 * 
	 * polyglot.ast.Expr condition = doStmt.cond(); soot.Value sootCond =
	 * base().createExpr(condition);
	 * 
	 * trueNoop.pop();
	 * 
	 * boolean needIf = needSootIf(sootCond); if (!(sootCond instanceof
	 * soot.jimple.ConditionExpr)) { sootCond =
	 * soot.jimple.Jimple.v().newNeExpr(sootCond, soot.jimple.IntConstant.v(0));
	 * } else { sootCond = handleDFLCond((soot.jimple.ConditionExpr)sootCond); }
	 * if (needIf){ soot.jimple.IfStmt ifStmt =
	 * soot.jimple.Jimple.v().newIfStmt(sootCond, noop1);
	 * body.getUnits().add(ifStmt); Util.addPosTag(ifStmt.getConditionBox(),
	 * condition.position()); Util.addLnPosTags(ifStmt, condition.position()); }
	 * else { soot.jimple.GotoStmt gotoIf =
	 * soot.jimple.Jimple.v().newGotoStmt(noop1); body.getUnits().add(gotoIf); }
	 * body.getUnits().add((soot.jimple.Stmt)(endControlNoop.pop()));
	 * condControlNoop.pop(); body.getUnits().add(falseNoop.pop()); }
	 */
	 ...
	 * condControlNoop.push(soot.jimple.Jimple.v().newNopStmt());
	 * 
	 * // handle for inits Iterator initsIt = forStmt.inits().iterator(); while
	 * (initsIt.hasNext()){ createStmt((polyglot.ast.Stmt)initsIt.next()); }
	 * soot.jimple.Stmt noop1 = soot.jimple.Jimple.v().newNopStmt();
	 * soot.jimple.Stmt noop2 = soot.jimple.Jimple.v().newNopStmt();
	 * 
	 * body.getUnits().add(noop2);
	 ...
	 * // handle cond
	 * 
	 * polyglot.ast.Expr condition = forStmt.cond(); if (condition != null) {
	 * trueNoop.push(tNoop); falseNoop.push(fNoop); soot.Value sootCond =
	 * base().createExpr(condition); trueNoop.pop(); falseNoop.pop();
	 * 
	 * boolean needIf = needSootIf(sootCond); if (!(sootCond instanceof
	 * soot.jimple.ConditionExpr)) { sootCond =
	 * soot.jimple.Jimple.v().newEqExpr(sootCond, soot.jimple.IntConstant.v(0));
	 * } else { sootCond =
	 * reverseCondition((soot.jimple.ConditionExpr)sootCond); sootCond =
	 * handleDFLCond((soot.jimple.ConditionExpr)sootCond); } if (needIf){
	 * soot.jimple.IfStmt ifStmt = soot.jimple.Jimple.v().newIfStmt(sootCond,
	 * noop1);
	 * 
	 * // add cond body.getUnits().add(ifStmt);
	 * 
	 * // add line and pos tags Util.addLnPosTags(ifStmt.getConditionBox(),
	 * condition.position()); Util.addLnPosTags(ifStmt, condition.position()); }
	 * //else { // soot.jimple.GotoStmt gotoIf =
	 * soot.jimple.Jimple.v().newGotoStmt(noop1); //
	 * body.getUnits().add(gotoIf); //}
	 * 
	 * } //else { // soot.jimple.Stmt goto2 =
	 * soot.jimple.Jimple.v().newGotoStmt(noop1); // body.getUnits().add(goto2);
	 * 
	 * //}
	 * 
	 * 
	 * // handle body //soot.jimple.Stmt noop2 =
	 * soot.jimple.Jimple.v().newNopStmt(); //soot.jimple.Stmt goto1 =
	 * soot.jimple.Jimple.v().newGotoStmt(noop2); //body.getUnits().add(goto1);
	 * //body.getUnits().add(noop1); body.getUnits().add(tNoop);
	 ...
	 * // handle label continue //if ((labelContinueMap != null) &&
	 * (labelContinueMap.containsKey(lastLabel))){ if (labelMap != null &&
	 * labelMap.containsKey(forStmt)){
	 * body.getUnits().add((soot.jimple.Stmt)labelMap.get(forStmt)); }
	 * 
	 * /*if ((labelContinueMap != null) && (labelStack != null) &&
	 * (!labelStack.isEmpty()) &&
	 * (labelContinueMap.containsKey(((LabelKey)labelStack.peek()).label()))){
	 * body.getUnits().add((soot.jimple.Stmt)labelContinueMap.get(((LabelKey)
	 * labelStack.peek()).label()));
	 * //System.out.println("lastLabel: "+lastLabel); //if
	 * (!body.getUnits().contains((soot.jimple.Stmt)labelContinueMap.get(
	 * lastLabel))){ //
	 ...
	 * //System.out.println("for iters: "+forStmt.iters()); while
	 * (itersIt.hasNext()){ createStmt((polyglot.ast.Stmt)itersIt.next()); }
	 * soot.jimple.Stmt goto1 = soot.jimple.Jimple.v().newGotoStmt(noop2);
	 * body.getUnits().add(goto1); //body.getUnits().add(noop2);
	 * 
	 * // handle cond
	 * 
	 * /*polyglot.ast.Expr condition = forStmt.cond(); if (condition != null) {
	 * soot.Value sootCond = base().createExpr(condition); boolean needIf =
	 * needSootIf(sootCond); if (!(sootCond instanceof
	 * soot.jimple.ConditionExpr)) { sootCond =
	 * soot.jimple.Jimple.v().newNeExpr(sootCond, soot.jimple.IntConstant.v(0));
	 * } else { sootCond = handleDFLCond((soot.jimple.ConditionExpr)sootCond); }
	 * if (needIf){ soot.jimple.IfStmt ifStmt =
	...
	private void createLocalDecl(polyglot.ast.LocalDecl localDecl) {
		...
		if (expr != null) {
			...
			if (localDecl.position() != null) {
				...
				if (expr != null) {
					Util.addLnPosTags(stmt, localDecl.position().line(), expr.position().endLine(),
							localDecl.position().column(), expr.position().endColumn());
				} else {
					Util.addLnPosTags(stmt, localDecl.position().line(), localDecl.position().endLine(),
	...
	private void createSwitch(polyglot.ast.Switch switchStmt) {
		...
		if (isLookupSwitch(switchStmt)) {
			...
			soot.jimple.LookupSwitchStmt lookupStmt = soot.jimple.Jimple.v().newLookupSwitchStmt(sootValue, values,
			...
			soot.jimple.TableSwitchStmt tableStmt = soot.jimple.Jimple.v().newTableSwitchStmt(sootValue, (int) lowVal,
					(int) highVal, sortedTargets, defaultTarget);
	...
	/**
	 * Determine if switch should be lookup or table - this doesn't always get
	 * the same result as javac lookup: non-table table: sequential (no gaps)
	 */
		...
		if (branchStmt.kind() == polyglot.ast.Branch.BREAK) {
			...
			if (branchStmt.label() == null) {
				...
				soot.jimple.Stmt gotoLabel = soot.jimple.Jimple.v()
						.newGotoStmt(labelContinueMap.get(branchStmt.label()));
				body.getUnits().add(gotoLabel);
				Util.addLnPosTags(gotoLabel, branchStmt.position());
			}
	...
	private void createLabeled(polyglot.ast.Labeled labeledStmt) {
		...
		/*
		 * else { if (labelStack == null){ labelStack = new Stack(); }
		...
		/*
		 * if (labelStack != null && !labelStack.isEmpty() && (stmt instanceof
		 * polyglot.ast.For || stmt instanceof polyglot.ast.Do)){
		 * labelStack.pop(); }
		 */
	 ...
	 * class LabelKey{
	 * 
	 * public LabelKey(String label, soot.jimple.Stmt noop){ this.label = label;
	 * this.noop = noop; } private String label; public String label(){ return
	 * label; } private soot.jimple.Stmt noop; public soot.jimple.Stmt noop(){
	 * return noop; } }
	 */
	...
	private void createAssert(polyglot.ast.Assert assertStmt) {
		...
		soot.SootFieldRef assertField = soot.Scene.v().makeFieldRef(body.getMethod().getDeclaringClass(),
				"$assertionsDisabled", soot.BooleanType.v(), true);
		...
		// actual cond test
		if ((assertStmt.cond() instanceof polyglot.ast.BooleanLit)
				&& (!((polyglot.ast.BooleanLit) assertStmt.cond()).value())) {
			// don't makeif
		} else {
		...
		methToInvoke = soot.Scene.v().makeMethodRef(soot.Scene.v().getSootClass("java.lang.AssertionError"), "<init>",
				paramTypes, soot.VoidType.v(), false);

		soot.jimple.SpecialInvokeExpr invokeExpr = soot.jimple.Jimple.v().newSpecialInvokeExpr(failureLocal,
				methToInvoke, params);
	...
	private void createSynchronized(polyglot.ast.Synchronized synchStmt) {
		...
		if (before.size() > 0) {
			addToExceptionList(startNoop, before.get(0), catchAllBeforeNoop,
	 ...
	 * instead if (expr.isConstant() && expr.constantValue() != null &&
	 * expr.type() != null && !(expr instanceof polyglot.ast.Binary &&
	 * expr.type().toString().equals("java.lang.String")) ){ return
	 * createConstant(expr); } if (expr instanceof polyglot.ast.Assign) { return
	 * getAssignLocal((polyglot.ast.Assign)expr); } else if (expr instanceof
	 * polyglot.ast.Lit) { return createLiteral((polyglot.ast.Lit)expr); } else
	 * if (expr instanceof polyglot.ast.Local) { return
	 * getLocal((polyglot.ast.Local)expr); } else if (expr instanceof
	 * polyglot.ast.Binary) { return getBinaryLocal((polyglot.ast.Binary)expr);
	 * } else if (expr instanceof polyglot.ast.Unary) { return
	 * getUnaryLocal((polyglot.ast.Unary)expr); } else if (expr instanceof
	 * polyglot.ast.Cast) { return getCastLocal((polyglot.ast.Cast)expr); }
	 * //else if (expr instanceof polyglot.ast.ArrayInit) { // array init are
	 * special and get created elsewhere //} else if (expr instanceof
	 * polyglot.ast.ArrayAccess) { return
	 * getArrayRefLocal((polyglot.ast.ArrayAccess)expr); } else if (expr
	 * instanceof polyglot.ast.NewArray) { return
	 * getNewArrayLocal((polyglot.ast.NewArray)expr); } else if (expr instanceof
	 * polyglot.ast.Call) { return getCallLocal((polyglot.ast.Call)expr); } else
	 * if (expr instanceof polyglot.ast.New) { return
	 * getNewLocal((polyglot.ast.New)expr); } else if (expr instanceof
	 * polyglot.ast.Special) { return
	 * getSpecialLocal((polyglot.ast.Special)expr); } else if (expr instanceof
	 * polyglot.ast.Instanceof) { return
	 * getInstanceOfLocal((polyglot.ast.Instanceof)expr); } else if (expr
	 * instanceof polyglot.ast.Conditional) { return
	 * getConditionalLocal((polyglot.ast.Conditional)expr); } else if (expr
	 * instanceof polyglot.ast.Field) { return
	 * getFieldLocal((polyglot.ast.Field)expr); } else { throw new
	 * RuntimeException("Unhandled Expression: "+expr); }
	 * 
	 * }
	 */
	 ...
	 */
	protected soot.Value createAggressiveExpr(polyglot.ast.Expr expr, boolean reduceAggressively,
	...
	private soot.SootMethod addSetAccessMeth(soot.SootClass conClass, polyglot.ast.Field field, soot.Value param) {
		if ((InitialResolver.v().getPrivateFieldSetAccessMap() != null) && (InitialResolver.v()
				.getPrivateFieldSetAccessMap().containsKey(new polyglot.util.IdentityKey(field.fieldInstance())))) {
			return InitialResolver.v().getPrivateFieldSetAccessMap()
					.get(new polyglot.util.IdentityKey(field.fieldInstance()));
		}
		String name = "access$" + soot.javaToJimple.InitialResolver.v().getNextPrivateAccessCounter() + "00";
		ArrayList paramTypes = new ArrayList();
		 ...
		 * paramTypes.add(soot.RefType.v("java.lang.Object")); retType =
		 * soot.RefType.v("java.lang.Object"); } else {
		 * paramTypes.add(param.getType()); retType = param.getType(); }
		 */
		soot.SootMethod meth = Scene.v().makeSootMethod(name, paramTypes, retType, soot.Modifier.STATIC);
		PrivateFieldSetMethodSource pfsms = new PrivateFieldSetMethodSource(Util.getSootType(field.type()),
				field.name(), field.flags().isStatic());
	...
	private soot.SootMethod addGetFieldAccessMeth(soot.SootClass conClass, polyglot.ast.Field field) {
		if ((InitialResolver.v().getPrivateFieldGetAccessMap() != null) && (InitialResolver.v()
				.getPrivateFieldGetAccessMap().containsKey(new polyglot.util.IdentityKey(field.fieldInstance())))) {
			return InitialResolver.v().getPrivateFieldGetAccessMap()
					.get(new polyglot.util.IdentityKey(field.fieldInstance()));
		}
		String name = "access$" + soot.javaToJimple.InitialResolver.v().getNextPrivateAccessCounter() + "00";
		ArrayList paramTypes = new ArrayList();
		...
		soot.SootMethod meth = Scene.v().makeSootMethod(name, paramTypes, Util.getSootType(field.type()),
				soot.Modifier.STATIC);
		PrivateFieldAccMethodSource pfams = new PrivateFieldAccMethodSource(Util.getSootType(field.type()),
				field.name(), field.flags().isStatic(), conClass);
	...
	private soot.SootMethod addGetMethodAccessMeth(soot.SootClass conClass, polyglot.ast.Call call) {
		if ((InitialResolver.v().getPrivateMethodGetAccessMap() != null) && (InitialResolver.v()
				.getPrivateMethodGetAccessMap().containsKey(new polyglot.util.IdentityKey(call.methodInstance())))) {
			return InitialResolver.v().getPrivateMethodGetAccessMap()
					.get(new polyglot.util.IdentityKey(call.methodInstance()));
		}
		String name = "access$" + soot.javaToJimple.InitialResolver.v().getNextPrivateAccessCounter() + "00";
		ArrayList paramTypes = new ArrayList();
		...
		soot.SootMethod meth = Scene.v().makeSootMethod(name, paramTypes,
				Util.getSootType(call.methodInstance().returnType()), soot.Modifier.STATIC);
	...
	protected soot.Value getAssignLocal(polyglot.ast.Assign assign) {
		...
		if (base().needsAccessor(assign.left())) {
			...
			// &&
			// !Util.getSootType(((polyglot.ast.Field)assign.left()).fieldInstance().container()).equals(body.getMethod().getDeclaringClass().getType())){
	...
	private soot.Value getFieldLocal(polyglot.ast.Field field) {
		...
		polyglot.ast.Receiver receiver = field.target();
		soot.javaToJimple.PolyglotMethodSource ms = (soot.javaToJimple.PolyglotMethodSource) body.getMethod()
				.getSource();
	 ...
	 */
	/*
	 * protected boolean needsPrivateAccessor(polyglot.ast.Field field){ if
	 * (field.fieldInstance().flags().isPrivate()){ if
	 * (!Util.getSootType(field.fieldInstance().container()).equals(body.
	 * getMethod().getDeclaringClass().getType())){ return true; } } return
	 * false; }
	 */

	/**
	 * needs a protected access method if field is protected and in a super
	 * class of the outer class of the innerclass trying to access the field (ie
	 * not in self or in outer of self)
	 */
	/*
	 * protected boolean needsProtectedAccessor(polyglot.ast.Field field){
	 * //return false; if (field.fieldInstance().flags().isProtected()){ if
	 * (Util.getSootType(field.fieldInstance().container()).equals(body.
	 * getMethod().getDeclaringClass().getType())){ return false; }
	 * soot.SootClass currentClass = body.getMethod().getDeclaringClass(); while
	 * (currentClass.hasOuterClass()){ currentClass =
	 * currentClass.getOuterClass(); if
	 * (Util.getSootType(field.fieldInstance().container()).equals(currentClass.
	 * getType())){ return false; } else if
	 * (Util.getSootType(field.fieldInstance().container()).equals(currentClass.
	 * getSuperclass().getType())){ return true; } } return false; } return
	 * false; /* if (field.fieldInstance().flags().isProtected()){ if
	 * (!Util.getSootType(field.fieldInstance().container()).equals(body.
	 * getMethod().getDeclaringClass().getType())){ soot.SootClass checkClass =
	 * body.getMethod().getDeclaringClass(); while (checkClass.hasOuterClass()){
	 * checkClass = checkClass.getOuterClass(); if
	 * (Util.getSootType(field.fieldInstance().container()).equals(checkClass.
	...
	protected soot.jimple.FieldRef getFieldRef(polyglot.ast.Field field) {
		...
		soot.SootFieldRef receiverField = soot.Scene.v().makeFieldRef(receiverClass, field.name(),
				Util.getSootType(field.type()), field.flags().isStatic());
	...
	private soot.Local getPrivateAccessFieldLocal(polyglot.ast.Field field, soot.Value base) {
		...
		if (field.fieldInstance().flags().isPrivate()) {
			toInvoke = addGetFieldAccessMeth(
					((soot.RefType) Util.getSootType(field.fieldInstance().container())).getSootClass(), field);
			...
			soot.SootClass containingClass = ((soot.RefType) Util.getSootType(field.fieldInstance().container()))
					.getSootClass();
		...
		if (!field.fieldInstance().flags().isStatic()) {
			 ...
			 * else if
			 * (body.getMethod().getDeclaringClass().declaresFieldByName(
			 * "this$0")){ params.add(getThis(invokeClass.getType())); } else {
			 * soot.Local local =
			 * (soot.Local)base().getBaseLocal(field.target());
			 * params.add(local); }
			 */

			// (soot.Local)getBaseLocal(field.target()));
			/*
			 * if (Util.getSootType(field.target().type()).equals(invokeClass.
			 * getType())){
			 */
	...
	private soot.Local getSpecialClassLitLocal(polyglot.ast.ClassLit lit) {
		...
		if (lit.typeNode().type().isPrimitive()) {
			...
			if (thisClass.isInterface()) {
				...
				if ((specialAnonMap != null) && (specialAnonMap.containsKey(thisClass))) {
					soot.SootClass specialClass = specialAnonMap.get(thisClass);
					invokeMeth = soot.Scene.v().makeMethodRef(specialClass, "class$", paramTypes,
							soot.RefType.v("java.lang.Class"), true);
				} else {
					throw new RuntimeException(
							"Class is interface so it must have an anon class to handle class lits but its anon class cannot be found.");
				...
				invokeMeth = soot.Scene.v().makeMethodRef(thisClass, "class$", paramTypes,
						soot.RefType.v("java.lang.Class"), true);
			}
			ArrayList params = new ArrayList();
	 ...
	 */
	private soot.Value getBinaryComparisonExpr(soot.Value lVal, soot.Value rVal,
			polyglot.ast.Binary.Operator operator) {
	...
	/**
	 * in bytecode and Jimple the conditions in conditional binary expressions
	 * are often reversed
	 */
	...
	private soot.Value createCondAnd(polyglot.ast.Binary binary) {
		...
		if (leftNeedIf) {
			...
			/*
			 * if (!falseNoop.empty()){ soot.jimple.Stmt fNoop =
			 * (soot.jimple.Stmt)falseNoop.peek(); ifLeft =
		...
		if (rightNeedIf) {
			...
			/*
			 * if (!falseNoop.empty()){ soot.jimple.Stmt fNoop =
			 * (soot.jimple.Stmt)falseNoop.peek(); ifRight =
	...
	private soot.Value createCondOr(polyglot.ast.Binary binary) {
		...
		if (leftNeedIf) {
			soot.jimple.IfStmt ifLeft;
			/*
			 * if (!trueNoop.empty()){ ifLeft =
			 * soot.jimple.Jimple.v().newIfStmt(lVal,
		...
		if (rightNeedIf) {
			...
			soot.jimple.IfStmt ifRight;
			/*
			 * if (!trueNoop.empty()){ if (inLeftOr == 0){ ifRight =
			 * soot.jimple.Jimple.v().newIfStmt(rVal,
			 * (soot.jimple.Stmt)falseNoop.peek()); } else { ifRight =
			 * soot.jimple.Jimple.v().newIfStmt(rVal,
			 * (soot.jimple.Stmt)trueNoop.peek()); } } else {
			 */
	...
	private soot.Local createStringBuffer(polyglot.ast.Expr expr) {
		...
		soot.SootMethodRef methodToInvoke1 = soot.Scene.v().makeMethodRef(classToInvoke1, "<init>", new ArrayList(),
				soot.VoidType.v(), false);
	...
	private soot.Local generateAppends(polyglot.ast.Expr expr, soot.Local sb) {
		...
		if (isStringConcat(expr)) {
			...
			soot.jimple.VirtualInvokeExpr appendInvoke = soot.jimple.Jimple.v().newVirtualInvokeExpr(sb, methodToInvoke,
					params);
	...
	private soot.Value getCastLocal(polyglot.ast.Cast castExpr) {
		...
		if (castExpr.expr().type().equals(castExpr.type()) || (castExpr.type().isClass()
				&& Util.getSootType(castExpr.type()).toString().equals("java.lang.Object"))) {
	 ...
	 * Adds extra params
	 */
	private void handleFinalLocalParams(ArrayList sootParams, ArrayList sootParamTypes,
			polyglot.types.ClassType keyType) {

		HashMap<IdentityKey, AnonLocalClassInfo> finalLocalInfo = soot.javaToJimple.InitialResolver.v()
				.finalLocalInfo();
		if (finalLocalInfo != null) {
			if (finalLocalInfo.containsKey(new polyglot.util.IdentityKey(keyType))) {
				...
				if (finalLocals != null) {
					...
					while (it.hasNext()) {
						Object next = it.next();
						polyglot.types.LocalInstance li = (polyglot.types.LocalInstance) ((polyglot.util.IdentityKey) next)
								.object();
	...
	private void createConstructorCall(polyglot.ast.ConstructorCall cCall) {
		...
		handleFinalLocalParams(sootParams, sootParamsTypes,
				(polyglot.types.ClassType) cCall.constructorInstance().container());

		soot.SootMethodRef methodToInvoke = getMethodFromClass(classToInvoke, "<init>", sootParamsTypes,
				soot.VoidType.v(), false);

		soot.jimple.SpecialInvokeExpr specialInvokeExpr = soot.jimple.Jimple.v().newSpecialInvokeExpr(base,
		...
		while (invokeParamsIt.hasNext()) {
			Util.addLnPosTags(specialInvokeExpr.getArgBox(numParams),
					((polyglot.ast.Expr) invokeParamsIt.next()).position());
			numParams++;
		}
	...
	private void handleFinalLocalInits() {
		...
		while (it.hasNext()) {
			...
			soot.jimple.AssignStmt stmt = soot.jimple.Jimple.v().newAssignStmt(fieldRef,
	...
	private soot.Local getNewLocal(polyglot.ast.New newExpr) {
		...
		if (newExpr.anonType() != null) {
			...
			if (!objType.isTopLevel()) {
				...
				polyglot.types.ClassType outerType = objType.outer();
				if (!InitialResolver.v().hasClassInnerTag(body.getMethod().getDeclaringClass(), name)) {
					Util.addInnerClassTag(body.getMethod().getDeclaringClass(), name,
							Util.getSootType(outerType).toString(), objType.name(), outerType.flags().isInterface()
									? soot.Modifier.PUBLIC | soot.Modifier.STATIC : Util.getModifier(objType.flags()));
				}
		...
		soot.SootMethodRef methodToInvoke = getMethodFromClass(classToInvoke, "<init>", sootParamsTypes,
				soot.VoidType.v(), false);
		soot.jimple.SpecialInvokeExpr specialInvokeExpr = soot.jimple.Jimple.v().newSpecialInvokeExpr(retLocal,
				methodToInvoke, sootParams);
	...
	private soot.Local getCallLocal(polyglot.ast.Call call) {
		...
		polyglot.types.MethodInstance methodInstance = call.methodInstance();
		/*
		 * soot.Type sootRetType =
		 * Util.getSootType(methodInstance.returnType()); ArrayList
		 * sootParamsTypes = getSootParamsTypes(call); ArrayList sootParams =
		 * getSootParams(call);
		 * 
		 * soot.SootMethodRef callMethod =
		 * soot.Scene.v().makeMethodRef(receiverTypeClass,
		 * methodInstance.name(), sootParamsTypes, sootRetType,
		...
		if (needsAccessor(call)) {
			...
			soot.SootClass containingClass = ((soot.RefType) Util.getSootType(call.methodInstance().container()))
					.getSootClass();
			...
			if (call.methodInstance().flags().isProtected()) {
				...
				if (body.getMethod().getDeclaringClass().hasOuterClass()) {
					...
					while (!InitialResolver.v().hierarchy().canStoreType(containingClass.getType(),
							addToClass.getType())) {
						if (addToClass.hasOuterClass()) {
							addToClass = addToClass.getOuterClass();
						} else {
	...
	private soot.Local getNewArrayLocal(polyglot.ast.NewArray newArrExpr) {
		...
		if (newArrExpr.numDims() == 1) {
			...
			soot.jimple.NewArrayExpr newArrayExpr = soot.jimple.Jimple.v()
					.newNewArrayExpr(((soot.ArrayType) sootType).getElementType(), dimLocal);
			expr = newArrayExpr;
			if (newArrExpr.additionalDims() != 1) {
			...
			soot.jimple.NewMultiArrayExpr newMultiArrayExpr = soot.jimple.Jimple.v()
					.newNewMultiArrayExpr((soot.ArrayType) sootType, valuesList);

			expr = newMultiArrayExpr;
			Iterator sizeBoxIt = newArrExpr.dims().iterator();
			int counter = 0;
			while (sizeBoxIt.hasNext()) {
				Util.addLnPosTags(newMultiArrayExpr.getSizeBox(counter),
						((polyglot.ast.Expr) sizeBoxIt.next()).position());
				counter++;
			}
		}
	...
	private soot.Local getArrayInitLocal(polyglot.ast.ArrayInit arrInit, polyglot.types.Type lhsType) {
		...
		soot.jimple.NewArrayExpr arrExpr = soot.jimple.Jimple.v().newNewArrayExpr(
				((soot.ArrayType) local.getType()).getElementType(),
				soot.jimple.IntConstant.v(arrInit.elements().size()));
			...
			if (elemExpr instanceof polyglot.ast.ArrayInit) {
				...
				if (((polyglot.ast.ArrayInit) elemExpr).type() instanceof polyglot.types.NullType) {
					if (lhsType instanceof polyglot.types.ArrayType) {
						...
						elem = getArrayInitLocal((polyglot.ast.ArrayInit) elemExpr,
								((polyglot.types.ArrayType) lhsType).base());
					} else {
	...
	private soot.Local getSpecialSuperQualifierLocal(polyglot.ast.Expr expr) {
		...
		if (expr instanceof polyglot.ast.Call) {
			...
			throw new RuntimeException(
					"Trying to create special super qualifier for: " + expr + " which is not a field or call");
		}
		...
		soot.jimple.InvokeExpr invokeExpr = soot.jimple.Jimple.v().newStaticInvokeExpr(methToInvoke.makeRef(),
				methodParams);
	...
	private soot.SootMethod makeSuperAccessMethod(soot.SootClass classToInvoke, Object memberToAccess) {
		...
		if (memberToAccess instanceof polyglot.ast.Field) {
			...
			meth = Scene.v().makeSootMethod(name, paramTypes, Util.getSootType(fieldToAccess.type()),
					soot.Modifier.STATIC);
			...
			meth = Scene.v().makeSootMethod(name, paramTypes,
					Util.getSootType(methToAccess.methodInstance().returnType()), soot.Modifier.STATIC);
	 ...
	 */
	/*
	 * private boolean isLitOrLocal(polyglot.ast.Expr exp) { if (exp instanceof
	 * polyglot.ast.Lit) return true; if (exp instanceof polyglot.ast.Local)
