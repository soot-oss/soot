 ...
 * 
 * In this case, the first trap covers the block and jumps to handler 1. The second trap also covers the block and jumps to handler 2. The third trap
 * covers handler 1 and jumps to handler 2. If we treat "any" as java.lang. Throwable, the second handler is clearly unnecessary. Worse, it violates
 * Soot's invariant that there may only be one handler per combination of covered code region and jump target.
 * 
...
public class DuplicateCatchAllTrapRemover extends BodyTransformer {
  ...
  /**
   * Checks whether the given trap covers the given unit, i.e., there is an exceptional control flow from the given unit to the given trap
   * 
   * @param b
