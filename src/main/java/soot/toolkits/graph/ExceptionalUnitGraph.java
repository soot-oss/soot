...
/**
 * <p>
 * Represents a control flow graph for a {@link Body} instance where the nodes are {@link Unit} instances, and where control flow associated with
 * exceptions is taken into account.
 * </p>
 *
 * <p>
 * To describe precisely the circumstances under which exceptional edges are added to the graph, we need to distinguish the exceptions thrown
 * explicitly by a <code>throw</code> instruction from the exceptions which are thrown implicitly by the VM to signal an error it encounters in the
 * course of executing an instruction, which need not be a <code>throw</code>.
 * </p>
 * 
 * <p>
 * For every {@link ThrowInst} or {@link ThrowStmt} <code>Unit</code> which may explicitly throw an exception that would be caught by a {@link Trap}
 * in the <code>Body</code>, there will be an edge from the <code>throw</code> <code>Unit</code> to the <code>Trap</code> handler's first
 * <code>Unit</code>.
 * </p>
 *
 * <p>
 * For every <code>Unit</code> which may implicitly throw an exception that could be caught by a <code>Trap</code> in the <code>Body</code>, there
 * will be an edge from each of the excepting <code>Unit</code>'s predecessors to the <code>Trap</code> handler's first <code>Unit</code> (since any
 * of those predecessors may have been the last <code>Unit</code> to complete execution before the handler starts execution). If the excepting
 * <code>Unit</code> might have the side effect of changing some field, then there will definitely be an edge from the excepting <code>Unit</code>
 * itself to its handlers, since the side effect might occur before the exception is raised. If the excepting <code>Unit</code> has no side effects,
 * then parameters passed to the <code>ExceptionalUnitGraph</code> constructor determine whether or not there is an edge from the excepting
 * <code>Unit</code> itself to the handler <code>Unit</code>.
 * </p>
 */
public class ExceptionalUnitGraph extends UnitGraph implements ExceptionalGraph<Unit> {
  ...
  /**
   * Constructs the graph for a given Body instance, using the <code>ThrowAnalysis</code> and <code>omitExceptingUnitEdges</code> value that are
   ...
   *          indicates whether the CFG should omit edges to a handler from trapped <code>Unit</code>s which may implicitly throw an exception which
   *          the handler catches but which have no potential side effects. The CFG will contain edges to the handler from all predecessors of
   *          <code>Unit</code>s which may implicitly throw a caught exception regardless of the setting for this parameter. If this parameter is
   *          <code>false</code>, there will also be edges to the handler from all the potentially excepting <code>Unit</code>s themselves. If this
   *          parameter is <code>true</code>, there will be edges to the handler from the excepting <code>Unit</code>s themselves only if they have
   *          potential side effects (or if they are themselves the predecessors of other potentially excepting <code>Unit</code>s). A setting of
   *          <code>true</code> produces CFGs which allow for more precise analyses, since a <code>Unit</code> without side effects has no effect on
   *          the computational state when it throws an exception. Use settings of <code>false</code> for compatibility with more conservative
  ...
  /**
   * Constructs the graph from a given Body instance using the passed {@link ThrowAnalysis} and a default value, provided by the {@link Options}
   * class, for the <code>omitExceptingUnitEdges</code> parameter.
   *
   * @param body
  ...
  /**
   * Constructs the graph from a given Body instance, using the {@link Scene} 's default {@link ThrowAnalysis} to estimate the set of exceptions that
   * each {@link Unit} might throw and a default value, provided by the {@link Options} class, for the <code>omitExceptingUnitEdges</code> parameter.
   *
   * @param body
   *          the <code>Body</code> from which to build a graph.
   *
  ...
  /**
   * <p>
   * Allocates an <code>ExceptionalUnitGraph</code> object without initializing it. This &ldquo;partial constructor&rdquo; is provided for the benefit
   * of subclasses whose constructors need to perform some subclass-specific processing before actually creating the graph edges (because, for
   * example, the subclass overrides a utility method like {@link #buildExceptionDests(ThrowAnalysis)} or
   * {@link #buildExceptionalEdges(ThrowAnalysis, Map, Map, Map, boolean)} with a replacement method that depends on additional parameters passed to
   * the subclass's constructor). The subclass constructor is responsible for calling {@link #initialize(ThrowAnalysis, boolean)}, or otherwise
   * performing the initialization required to implement <code>ExceptionalUnitGraph</code>'s interface.
   * </p>
   *
   * <p>
   * Clients who opt to extend <code>ExceptionalUnitGraph</code> should be warned that the class has not been carefully designed for inheritance; code
   * that uses the <code>protected</code> members of this class may need to be rewritten for each new Soot release.
   * </p>
   *
   * @param body
   *          the <code>Body</code> from which to build a graph.
   ...
   *          a meaningless placeholder, which exists solely to distinguish this constructor from the public {@link #ExceptionalUnitGraph(Body)}
   *          constructor.
   */
  ...
  /**
   * Performs the real work of constructing an <code>ExceptionalUnitGraph</code>, factored out of the constructors so that subclasses have the option
   * to delay creating the graph's edges until after they have performed some subclass-specific initialization.
   *
   * @param throwAnalysis
   *          the source of information about the exceptions which each {@link Unit} may throw.
   *
   * @param omitExceptingUnitEdges
   *          indicates whether the CFG should omit edges to a handler from trapped <code>Unit</code>s which may throw an exception which the handler
   *          catches but which have no potential side effects.
   */
  protected void initialize(ThrowAnalysis throwAnalysis, boolean omitExceptingUnitEdges) {
    ...
    if (body.getTraps().size() == 0) {
      ...
      trapUnitsThatAreHeads = buildExceptionalEdges(throwAnalysis, unitToExceptionDests, unitToExceptionalSuccs, unitToExceptionalPreds,
  ...
  /**
   * <p>
   * Utility method used in the construction of {@link soot.toolkits.graph.UnitGraph UnitGraph} variants which include exceptional control flow. It
   * determines which {@link Unit}s may throw exceptions that would be caught by {@link Trap}s within the method.
   * </p>
   *
   * @param throwAnalysis
   *          The source of information about which exceptions each <code>Unit</code> may throw.
   *
   * @return <code>null</code> if no <code>Unit</code>s in the method throw any exceptions caught within the method. Otherwise, a {@link Map} from
   *         <code>Unit</code>s to {@link Collection}s of {@link ExceptionDest}s.
   *
   *         <p>
   *         The returned map has an idiosyncracy which is hidden from most client code, but which is exposed to subclasses extending
   *         <code>ExceptionalUnitGraph</code>. If a <code>Unit</code> throws one or more exceptions which are caught within the method, it will be
   *         mapped to a <code>Collection</code> of <code>ExceptionDest</code>s describing the sets of exceptions that the <code>Unit</code> might
   *         throw to each {@link Trap}. But if all of a <code>Unit</code>'s exceptions escape the method, it will be mapped to
   *         <code>null</code, rather than to a
   *         <code>Collection</code> containing a single <code>ExceptionDest</code> with a <code>null</code> trap. (The special case for
   *         <code>Unit</code>s with no caught exceptions allows <code>buildExceptionDests()</code> to ignore completely <code>Unit</code>s which are
   *         outside the scope of all <code>Trap</code>s.)
   *         </p>
   */
    ...
    for (Trap trap : body.getTraps()) {
      ...
      for (Iterator<Unit> unitIt = units.iterator(trap.getBeginUnit(), units.getPredOf(trap.getEndUnit())); unitIt.hasNext();) {
        ...
        if (!catchableAs.getCaught().equals(ThrowableSet.Manager.v().EMPTY)) {
          ...
          assert thrownSet.equals(catchableAs
              .getUncaught()) : "ExceptionalUnitGraph.buildExceptionDests(): catchableAs.caught == EMPTY, but catchableAs.uncaught != thrownSet"
  ...
  /**
   * A utility method for recording the exceptions that a <code>Unit</code> throws to a particular <code>Trap</code>. Note that this method relies on
   * the fact that the call to add escaping exceptions for a <code>Unit</code> will always follow all calls for its caught exceptions.
   *
   * @param map
   *          A <code>Map</code> from <code>Unit</code>s to <code>Collection</code>s of <code>ExceptionDest</code>s. <code>null</code> if no
   *          exceptions have been recorded yet.
   *
   * @param u
   ...
   *
   * @return a <code>Map</code> which whose contents are equivalent to the input <code>map</code>, plus the information that <code>u</code> throws
   *         <code>caught</code> to <code>t</code>.
   */
   ...
   *          A <code>Map</code> from {@link Unit}s to {@link Collection}s of {@link ExceptionalUnitGraph.ExceptionDest ExceptionDest}s which
   *          represent the handlers that might catch exceptions thrown by the <code>Unit</code>. This is an ``in parameter''.
   *
   * @param unitToSuccs
   *          A <code>Map</code> from <code>Unit</code>s to {@link List}s of <code>Unit</code>s. This is an ``out parameter'';
   *          <code>buildExceptionalEdges</code> will add a mapping from every <code>Unit</code> in the body that may throw an exception that could be
   *          caught by a {@link Trap} in the body to a list of its exceptional successors.
   *
   * @param unitToPreds
   *          A <code>Map</code> from <code>Unit</code>s to <code>List</code>s of <code>Unit</code>s. This is an ``out parameter'';
   *          <code>buildExceptionalEdges</code> will add a mapping from each handler unit that may catch an exception to the list of
   *          <code>Unit</code>s whose exceptions it may catch.
   * @param omitExceptingUnitEdges
   *          Indicates whether to omit exceptional edges from excepting units which lack side effects
   *
   * @return a {@link Set} of trap <code>Unit</code>s that might catch exceptions thrown by the first <code>Unit</code> in the {@link Body} associated
   *         with the graph being constructed. Such trap <code>Unit</code>s may need to be added to the list of heads (depending on your definition of
   *         heads), since they can be the first <code>Unit</code> in the <code>Body</code> which actually completes execution.
   */
  protected Set<Unit> buildExceptionalEdges(ThrowAnalysis throwAnalysis, Map<Unit, Collection<ExceptionDest>> unitToExceptionDests,
      Map<Unit, List<Unit>> unitToSuccs, Map<Unit, List<Unit>> unitToPreds, boolean omitExceptingUnitEdges) {
  ...
  /**
   * <p>
   * Utility method for checking if a {@link Unit} might have side effects. It simply returns true for any unit which invokes a method directly or
   * which might invoke static initializers indirectly (by creating a new object or by refering to a static field; see sections 2.17.4, 2.17.5, and
   * 5.5 of the Java Virtual Machine Specification).
   * </p>
   *
   * <code>mightHaveSideEffects()</code> is declared package-private so that it is available to unit tests that are part of this package.
   *
   * @param u
  ...
  /**
   * <p>
   * A placeholder that overrides {@link UnitGraph#buildHeadsAndTails()} with a method which always throws an exception. The placeholder serves to
   * indicate that <code>ExceptionalUnitGraph</code> does not use <code>buildHeadsAndTails()</code>, and to document the conditions under which
   * <code>ExceptionalUnitGraph considers a node to be a head or 
   * tail.
   * </p>
   *
   * <p>
   * <code>ExceptionalUnitGraph</code> defines the graph's set of heads to include the first {@link Unit} in the graph's body, together with the first
   * <code>Unit</code> in any exception handler which might catch an exception thrown by the first <code>Unit</code> in the body (because any of those
   * <code>Unit</code>s might be the first to successfully complete execution). <code>ExceptionalUnitGraph</code> defines the graph's set of tails to
   * include all <code>Unit</code>s which represent some variety of return bytecode or an <code>athrow</code> bytecode whose argument might escape the
   * method.
   * </p>
   */
  @Override
  ...
  /**
   * Utility method, to be called only after the unitToPreds and unitToSuccs maps have been built. It defines the graph's set of heads to include the
   * first {@link Unit} in the graph's body, together with all the <code>Unit</code>s in <code>additionalHeads</code>. It defines the graph's set of
   * tails to include all <code>Unit</code>s which represent some sort of return bytecode or an <code>athrow</code> bytecode which may escape the
   * method.
   */
  ...
  /**
   * Returns a collection of {@link ExceptionalUnitGraph.ExceptionDest ExceptionDest} objects which represent how exceptions thrown by a specified
   * unit will be handled.
   *
   * @param u
   *          The unit for which to provide exception information. ( <code>u</code> must be a <code>Unit</code>, though the parameter is declared as
   *          an <code>Object</code> to satisfy the interface of {@link soot.toolkits.graph.ExceptionalGraph ExceptionalGraph}.
   *
   * @return a collection of <code>ExceptionDest</code> objects describing the traps, if any, which catch the exceptions which may be thrown by
   *         <code>u</code>.
   */
  @Override
  ...
  /**
   * <p>
   * Return the {@link ThrowAnalysis} used to construct this graph, if the graph contains no {@link Trap}s, or <code>null</code> if the graph does
   * contain <code>Trap</code>s. A reference to the <code>ThrowAnalysis</code> is kept when there are no <code>Trap</code>s so that the graph can
   * generate responses to {@link #getExceptionDests(Object)} on the fly, rather than precomputing information that may never be needed.
   * </p>
   *
   * <p>
   * This method is package-private because it exposes a detail of the implementation of <code>ExceptionalUnitGraph</code> so that the
   * {@link soot.toolkits.graph.ExceptionalBlockGraph ExceptionalBlockGraph} constructor can cache the same <code>ThrowAnalysis</code> for the same
   * purpose.
   *
   * @return the {@link ThrowAnalysis} used to generate this graph if the graph contains no {@link Trap}s, or <code>null</code> if the graph contains
   *         one or more {@link Trap}s.
   */
