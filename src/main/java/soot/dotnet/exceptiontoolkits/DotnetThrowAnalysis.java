...
/**
 * ThrowAnalysis for .NET CIL Instructions
 * Described Exceptions are described in the ECMA-335 specification
 */
        ...
        return new UnitThrowAnalysis.ValueSwitch() {
            ...
            private void caseBinopDivExpr(BinopExpr expr) {
                ...
                if ((divisorType instanceof UnknownType)
                        || (divisorType instanceof IntegerType && (!(divisor instanceof IntConstant) || divisor.equals(INT_CONSTANT_ZERO)))
                        || (divisorType instanceof LongType && (!(divisor instanceof LongConstant) || divisor.equals(LONG_CONSTANT_ZERO)))
                        // floating points never throw an exception
                ) {
                    result = result.add(Scene.v().getRefType(DotnetBasicTypes.SYSTEM_DIVIDEBYZEROEXCEPTION));
                }
                result = result.add(Scene.v().getRefType(DotnetBasicTypes.SYSTEM_ARITHMETICEXCEPTION));
                // result = result.add(Scene.v().getRefType(DotnetBasicTypes.SYSTEM_OVERFLOWEXCEPTION)); //Implementation specific (Microsoft)
