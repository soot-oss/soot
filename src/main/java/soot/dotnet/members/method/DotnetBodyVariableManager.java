 ...
 * Manager for variables in a .NET Body (Store, initialization, etc.)
 */
public class DotnetBodyVariableManager {
    ...
    private void initLocalVariables(List<ProtoIlInstructions.IlVariableMsg> locals) {
        for (ProtoIlInstructions.IlVariableMsg v : locals) {
            ...
            // Normally, structs are not instantiated with new obj, but while visiting default_value instruction a pseudo constructor is called
            // create new valuetype "object"
            AssignStmt assignStmt = Jimple.v().newAssignStmt(variable, Jimple.v().newNewExpr(RefType.v(v.getType().getFullname())));
            mainJb.getUnits().add(assignStmt);
        }
    }
