...
public class DexAnnotation {
	...
	public void handleMethodAnnotation(Host h, Method method) {
		...
		if (doParam) {
			VisibilityParameterAnnotationTag tag = new VisibilityParameterAnnotationTag(parameters.size(),
	...
	private List<Tag> handleAnnotation(Set<? extends org.jf.dexlib2.iface.Annotation> annotations, String classType) {
		...
		for (Annotation a : annotations) {
			...
			if (atypes.equals("dalvik.annotation.AnnotationDefault")) {
				...
				if (eSize != 1)
					throw new RuntimeException(
							"error: expected 1 element for annotation EnclosingClass. Got " + eSize + " instead.");
				...
				if (eSize != 1)
					throw new RuntimeException(
							"error: expected 1 element for annotation EnclosingMethod. Got " + eSize + " instead.");
	...
	private ArrayList<AnnotationElem> handleAnnotationElement(AnnotationElement ae,
			List<? extends EncodedValue> evList) {
		ArrayList<AnnotationElem> aelemList = new ArrayList<AnnotationElem>();
		...
		for (EncodedValue ev : evList) {
			...
			case 0x1b: // ENUM : Warning -> encoding Dalvik specific!
			{
				...
				FieldReference fr = v.getValue();
				elem = new AnnotationEnumElem(DexType.toSootAT(fr.getType()).toString(), fr.getName(), 'e',
						ae.getName());
				break;
			}
			case 0x1c: // ARRAY
	 ...
	 * 
	 * In Dalvik: VISIBILITY_BUILD 0x00 intended only to be visible at build
	 * time (e.g., during compilation of other code) VISIBILITY_RUNTIME 0x01
	 * intended to visible at runtime VISIBILITY_SYSTEM 0x02 intended to visible
	 * at runtime, but only to the underlying system (and not to regular user
	 * code)
	 *
	 * @param visibility
	 *            Dexlib visibility
