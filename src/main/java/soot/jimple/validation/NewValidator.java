...
/**
 * A relatively simple validator. It tries to check whether after each
 * new-expression-statement there is a corresponding call to the &lt;init&gt;
...
public enum NewValidator implements BodyValidator {
	...
	/**
	 * <p>
	 * Checks whether all pathes from start to the end of the method have a call to
	 * the &lt;init&gt; method in between.
	 * </p>
	 * <code>
	 * $r0 = new X;<br>
	 * ...<br>
	 ...
	 * The first local in the set is always the local on the LHS of the
	 * new-expression-assignment (called: original local; in the example
	 * <code>$r0</code>).
	 * </p>
	 *
	 * @param g
				...
				if (curStmt.containsInvokeExpr()) {
					...
					if (expr.getMethod().isConstructor()) {
						if (!(expr instanceof SpecialInvokeExpr)) {
							exception.add(new ValidationException(curStmt,
