...
public class Walker extends DepthFirstAdapter {
   ...
   * semicolon | {exitmonitor} exitmonitor immediate semicolon | {switch} switch l_paren immediate r_paren l_brace
   * case_stmt+ r_brace semicolon | {identity} local_name colon_equals at_identifier type semicolon | {identity_no_type}
   * local_name colon_equals at_identifier semicolon | {assign} variable equals expression semicolon | {if} if bool_expr
   * goto_stmt | {goto} goto_stmt | {nop} nop semicolon | {ret} ret immediate? semicolon | {return} return immediate?
   * semicolon | {throw} throw immediate semicolon | {invoke} invoke_expr semicolon;
   */
