...
public class JavaLangClassNative extends NativeMethodClass {
   ...
   *
   * In theory, the same class may have two or more representations at the runtime. But statically, we can assume that all
   * variables of java.lang.Class type are aliased together. By looking at static class hierarchy, there is only one
   * ReferenceVariable variable for a class in the hierarchy.
   */

  /**
   * NOTE: the semantic of forName0 follows forName method.
   *
   * Returns the Class object associated with the class or interface with the given string name, using the given class
   * loader. Given the fully qualified name for a class or interface (in the same format returned by getName) this method
   * attempts to locate, load, and link the class or interface. The specified class loader is used to load the class or
   * interface. If the parameter loader is null, the class is loaded through the bootstrap class loader. The class is
   * initialized only if the initialize parameter is true and if it has not been initialized earlier.
   *
   * If name denotes a primitive type or void, an attempt will be made to locate a user-defined class in the unnamed package
   * whose name is name. Therefore, this method cannot be used to obtain any of the Class objects representing primitive
   * types or void.
   *
   * If name denotes an array class, the component type of the array class is loaded but not initialized.
   *
   * For example, in an instance method the expression: Class.forName("Foo") is equivalent to: Class.forName("Foo", true,
   ...
   *
   * it is almost impossible to distinguish the dynamic class loader for classes. a conservative way is to use one static
   * representation for all class loader, which means all class loader variable aliased together.
   *
  ...
  /**
   * Returns the Class representing the component type of an array. If this class does not represent an array class this
   * method returns null.
   *
  ...
  /**
   * Sets the signers of a class. This should be called after defining a class. Parameters: c - the Class object signers -
  ...
  /**
   * Gets the signers of this class. We need an artificial field variable to connect setSigners and getSigners.
   *
  ...
  /**
   * If the class or interface represented by this Class object is a member of another class, returns the Class object
   * representing the class in which it was declared. This method returns null if this class or interface is not a member of
   * any other class. If this Class object represents an array class, a primitive type, or void,then this method returns
   * null.
   *
   ...
   * represented by this Class object, including those declared by the class or interface and and those inherited from
  ...
  /**
   * Returns a Constructor object that reflects the specified public constructor of the class represented by this Class
   * object. The parameterTypes parameter is an array of Class objects that identify the constructor's formal parameter
  ...
  /**
   * Returns a Field object that reflects the specified public member field of the class or interface represented by this
   * Class object.
   *
   * Called by getField(String)
   *
   * NOTE: getField0(String name), since the name can be dynamically constructed, it may be not able to know exact field name
  ...
  /**
   * Returns a Method object that reflects the specified public member method of the class or interface represented by this
   * Class object.
   *
   * Called by getMethod()
   *
   ...
   *
   * private static native void registerNatives(); public native boolean isInstance(java.lang.Object); public native boolean
   * isAssignableFrom(java.lang.Class); public native boolean isInterface(); public native boolean isArray(); public native
   * boolean isPrimitive(); public native int getModifiers();
   */
