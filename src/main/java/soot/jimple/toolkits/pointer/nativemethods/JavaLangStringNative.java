...
public class JavaLangStringNative extends NativeMethodClass {
   ...
   * method, then the string from the pool is * returned. Otherwise, this String object is added to the pool and a * reference to this String object
   * is returned.
   *
   * It follows that for any two strings s and t, s.intern() == t.intern() is true if and only if s.equals(t) is true.
   *
   * All literal strings and string-valued constant expressions are * interned. String literals are defined in Section 3.10.5 of the Java * Language
   * Specification Returns: a string that has the same contents * as this string, but is guaranteed to be from a pool of unique * strings.
   *
   * Side Effect: from the description, we can see, it is tricky to know the side effect of this native method. Take a conservative way to handle
   * this.
   *
