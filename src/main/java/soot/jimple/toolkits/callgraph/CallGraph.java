...
/**
 * Represents the edges in a call graph. This class is meant to act as only a
 * container of edges; code for various call graph builders should be kept out
 * of it, as well as most code for accessing the edges.
 * 
...
public class CallGraph implements Iterable<Edge> {
	...
	/**
	 * Swaps an invocation statement. All edges that previously went from the given
	 * statement to some callee now go from the new statement to the same callee.
	 * This method is intended to be used when a Jimple statement is replaced, but
	 * the replacement does not semantically affect the edges.
	 * 
	 * @param out
	 *            The old statement
	 * @param in
	 ...
	 * call target is callee. Without advanced data structure, we can only
	 * sequentially search for the match. Fortunately, the number of outgoing edges
	 * for a unit is not too large.
	 * 
	 * @param u
	 * @param callee
	 * @return
