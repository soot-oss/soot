 ...
 * called. The inter-procedural analysis interacts with a intra-procedural
 * analysis that is able to compute the summary of one method, given the summary
 ...
 * represent the effect of the method from its entry point and up to the current
 * point. At the entry point, this value is empty. The summary of the method is
 * then the merge of the abstract values at all its return points.
 *
 * You can provide off-the-shelf summaries for methods you do not which to
 ...
 * This is useful for native methods, incremental analysis, or when you
 * hand-made summary. Methods that are called solely by filtered-out ones will
 * never be analysed, effectively trimming the call-graph dependencies.
 *
 * This class tries to use the same abstract methods and data management policy
 * as regular FlowAnalysis classes.
 *
 * @param <S>
 */
public abstract class AbstractInterproceduralAnalysis<S> {
     ...
     * consequence to {@link #analyseCall(Object, Stmt, Object)}, once for each
     * possible target method of the {@code callStmt}, provided with its
     * summary.
     *
     ...
     * summary o.
     *
     * @param prefix gives you a unique string to prefix your node names and
     *               avoid name-clash
     * @param o
     * @param out
     */
    ...
    public void drawAsManyDot(String prefix, boolean drawUnanalysed) {
        ...
        if (drawUnanalysed) {
            for (SootMethod m : unanalysed.keySet()) {
                ...
                File f = new File(SourceLocator.v().getOutputDir(),
                        prefix + m.toString() + "_u" + DotGraph.DOT_EXTENSION);
                dot.plot(f.getPath());
            }
        }
    }
