...
public class LoopConditionUnroller extends BodyTransformer {
	 ...
	 * changes (ie. a trap opens or closes), an entry is added in the map, and
	 * the unit is mapped to the trap. The values associated to the keys are
	 * lists, as more than one exception can change at a unit.<br>
	 * Even if a trap opens and closes at a unit, this trap is only reported
	 * once (ie. is only once in the list).
	 *
	 ...
	 * modify the exception-chain). Furthermore the inserted block will not
	 * change the behaviour of the program.<br>
	...
	private Unit copyBlock(Block block) {
		...
		for (Unit currentUnit = block.getHead(); currentUnit != newGoto; currentUnit = (Unit) unitChain
				.getSuccOf(currentUnit)) {
			...
			/*
			 * the traps...: if a trap is closed (in the original block) that
			 * hasn't been opened before, we have to open it at the beginning of
			 * the copied block. If a trap gets opened, but not closed, we only
			 * have to close it at the end of the (original) block (as it will
			 * be open at the end of the copied block.)
			 */
		...
		for (Block succ : block.getSuccs()) {
			if (visitedBlocks.contains(succ)) {
				if (succ != block && visitingSuccs.contains(succ)) {
					/*
					 * we only want blocks with at least 2 predecessors, to
					 * avoid that a copied while-condition gets copied again in
					 * a future pass of unrollConditions
					 */
					if (succ.getPreds().size() >= 2
							&& succ.getSuccs().size() == 2) {
						...
						if (getSize(condition) <= maxSize) {
							...
							if (loopTail instanceof GotoStmt)
								...
								if (((IfStmt) loopTail).getTarget() == condition
										.getHead())
