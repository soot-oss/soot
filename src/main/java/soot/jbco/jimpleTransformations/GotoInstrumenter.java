...
public class GotoInstrumenter extends BodyTransformer implements IJbcoTransform {
    ...
    protected void internalTransform(Body body, String phaseName, Map<String, String> options) {
        ...
        if (soot.jbco.Main.getWeight(phaseName, body.getMethod().getSignature()) == 0) return;
        ...
        if (unitsLeft < 8) {
            if (isVerbose()) {
                logger.info("Skipping {} method GOTO instrumentation as it is too small.",
                        body.getMethod().getSignature());
            }
            return;
        }
        ...
        if (tries >= MAX_TRIES_TO_GET_REORDER_COUNT) return;
        ...
        for (int reorder = 0; reorder < unitsQuantityToReorder; reorder++) {
            ...
            final UnitBox pointingToReorderingUnit[] = reorderingUnit.getBoxesPointingToThis()
                    .toArray(EMPTY_UNIT_BOX_ARRAY);
        ...
        final Unit firstReorderingNotGotoStmt = first instanceof GotoStmt
                ? ((GotoStmt) first).getTargetBox().getUnit() : firstReorderingUnit;
        ...
        final Unit caughtExceptionHandler = Jimple.v().newIdentityStmt(
                caughtExceptionLocal, Jimple.v().newCaughtExceptionRef());
        units.add(caughtExceptionHandler);
        units.add(Jimple.v().newThrowStmt(caughtExceptionLocal));

        final Iterator<Unit> reorderedUnitsIterator = units.iterator(
                secondReorderedUnit, units.getPredOf(caughtExceptionHandler));
        ...
        body.getTraps().add(
                Jimple.v().newTrap(
                        ...
                        units.getPredOf(firstReorderingNotGotoStmt),
                        trapEndUnit,
