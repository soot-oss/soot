...
public class Util {
	...
	/**
	 * Set the informations relative to the current method body. This method
	 * must be called before using getLocalForIndex(...) and
	 * getLocalForStackOp(...) each time a different current method body is
	 * considered.
	 */
	...
	public void resolveFromClassFile(SootClass aClass, InputStream is, String filePath, Collection<Type> references) {
		...
		// Load up class file, and retrieve bclass from class manager.
		{
			...
			if (!name.equals(bclass.getName())) {
				throw new RuntimeException("Error: class " + name + " read in from a classfile in which "
						+ bclass.getName() + " was expected.");
			}
		}
		...
		for (int i = 0; i < coffiClass.fields_count; i++) {
			...
			String fieldDescriptor = ((CONSTANT_Utf8_info) (coffiClass.constant_pool[fieldInfo.descriptor_index]))
					.convert();
		...
		for (int i = 0; i < coffiClass.methods_count; i++) {
			...
			String methodDescriptor = ((CONSTANT_Utf8_info) (coffiClass.constant_pool[methodInfo.descriptor_index]))
					.convert();
				...
				for (int j = 0; j < methodInfo.attributes_count; j++) {
					if (methodInfo.attributes[j] instanceof Exception_attribute) {
						...
						for (int k = 0; k < exceptions.number_of_exceptions; k++) {
							...
							String exceptionName = ((CONSTANT_Utf8_info) (coffiClass.constant_pool[c.name_index]))
									.convert();
						...
						addAnnotationVisibilityParameterAttribute(method, methodInfo.attributes[j], coffiClass,
								references);
	...
	String getNextEasyName() {
		final String[] easyNames = { "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p",
				"q", "r", "s", "t", "u", "v", "w", "x", "y", "z" };
	...
	private Local getLocalForIndex(JimpleBody listBody, int index, int bcIndex, int nextBcIndex, boolean isLocalStore) {
		...
		if (useFaithfulNaming && activeVariableTable != null) {
			 ...
			 * [use-original-names]
			 * 
			 * Use original variables names. Generally speeking, the current way
			 * of handling original variables names is sound if the local
			 ...
			 * ((s,l),i,N) means "from bytecode index s included to bytecode
			 * index s+l excluded, the variable at index i is named N is the
			 * source code".
			 * 
			 * However the content of the table is for informational and
			 * debugging purpose only, the compiler can insert whatever
			 ...
			 * with different indexes and overlapping bytecode ranges are given
			 * the same name in the table. Although it won't happen with
			 * user-defined variables since Java doesn't allow the same variable
			 * name to be declared in nested scopes. But this situation can
			 * arise for compiler-generated local variables if they are assigned
			 * the same name (`i$` is common in OpenJDK). Notable example are
			 * local variables generated to implement the Java for-each
			 * statement (for(Foo foo : bar){}). If the source code contains
			 * nested for-each statements, then it becomes ugly because all the
			 * generated iterators or counters (for-each on arrays) are assigned
			 * the same name in the table.
			 * 
			 * This inconsistancy is now handled correctly by the following
			 * code. The idea is based on the observation that the Local
			 * allocation works well if we simply allocate a different Local
			 * object for each local variable index (it is the default
			 * allocation policy, when the original names are not kept).
			 * Therefore, local variables with the same name and same index
			 * should have the same Local object. And local variables with the
			 * same name but different index should have a different Local
			 * object. We maintain the map nameToIndexToLocal :: Name -> (Index
			 * -> Local Object) from variable names to maps from variable
			 * indexes to Local objects. So we allocate a new Local object for
			 * each pair of a variable name and a variable index.
			 * 
			 ...
			 * names into account... we should not allocate Local according to
			 * these names. Instead we must keep the default allocation policy
			 * (one Local object for each local variable index), and then
			 * annotate each statement with "most probable name(s)" for each use
			 * or def Local.
			 * 
			 */
			if (bcIndex != -1) {
				...
				/*
				 * For local store bytecode, the local actually takes its new
				 * value after the bytecode is executed, so we must look at the
				...
				/*
				 * // for debug purpose String desc =
				 * activeVariableTable.getLocalVariableDescriptor(
				 * activeConstantPool, index, activeOriginalIndex); if
				 * (activeVariableTypeTable != null){ String debug_type =
				 * activeVariableTypeTable.getLocalVariableType(
				 * activeConstantPool, index, activeOriginalIndex); }
				 */
			}
		}
	...
	/*
	 * void setLocalType(Local local, List locals, int localIndex, Type type) {
	 * if(local.getType().equals(UnknownType.v()) ||
	 * local.getType().equals(type)) { local.setType(type);
	 * 
	 * if(local.getType().equals(DoubleType.v()) ||
	 * local.getType().equals(LongType.v())) { // This means the next local
	 ...
	 * return; }
	 * 
	 * if(type.equals(IntType.v())) { if(local.getType().equals(BooleanType.v())
	 * || local.getType().equals(CharType.v()) ||
	 * local.getType().equals(ShortType.v()) ||
	 * local.getType().equals(ByteType.v())) { // Even though it's not the same,
	 * it's ok, because booleans, chars, shorts, and // bytes are all sort of
	 * treated like integers by the JVM. return; }
	 * 
	 * }
	 * 
	 * throw new RuntimeException("required and actual types do not match: " +
	 * type.toString() + " with " + local.getType().toString()); }
	 */

	/**
	 * Verifies the prospective name for validity as a Jimple name. In
	 * particular, first-char is alpha | _ | $, subsequent-chars are alphanum |
	 * _ | $.
	 *
	 * We could use isJavaIdentifier, except that Jimple's grammar doesn't
	 * support all of those, just ASCII.
	 *
	...
	private void addAnnotations(int numAnnots, annotation[] annotations, ClassFile coffiClass,
			VisibilityAnnotationTag tag, Collection<Type> references) {
