		...
		if (bodies.size() == 1) { // this should always be one since there is
			...
			if (body.size() == 1) {// size is good
				...
				if (bodyNode instanceof ASTIfNode) {
					/*
					 * We can do AndAggregation at this point node contains the
					 * outer if (ASTIfNode)bodyNode is the inner if
					 */

					ASTCondition outerCond = node.get_Condition();
					ASTCondition innerCond = ((ASTIfNode) bodyNode)
							.get_Condition();

					SETNodeLabel outerLabel = (node).get_Label();
					SETNodeLabel innerLabel = ((ASTIfNode) bodyNode)
							.get_Label();

					SETNodeLabel newLabel = null;
					if (outerLabel.toString() == null
							&& innerLabel.toString() == null) {
						newLabel = outerLabel;
					} else if (outerLabel.toString() != null
							&& innerLabel.toString() == null) {
						newLabel = outerLabel;
					} else if (outerLabel.toString() == null
							&& innerLabel.toString() != null) {
						...
						// of outerlabel now
						changeUses(outerLabel.toString(),
								innerLabel.toString(), bodyNode);
					}
		...
		while (it.hasNext()) {
			...
			if (node instanceof ASTStatementSequenceNode) {
				...
				if (node instanceof ASTTryNode) {
					ASTTryNode.container subBody = (ASTTryNode.container) it
							.next();
