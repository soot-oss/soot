...
\begin{abstract}
  ...
  graphs. Since the full Soot API in our experience leaves novice
  users in a state of shock and awe, these notes also contain the
  important code snippets that make everything work.
\end{abstract}
...
projects involving different forms of analyses. Soot is a large
...
of this manual we have used version 2.2.2 of Soot, but have not tested
the instructions contained herein on the new release (2.2.3). To use
Soot these three Jars need to be in your classpath. To test the setup,
try executing:
\begin{center}
  \code{java soot.Main --help}
\end{center}
...
to use the Eclipse plugin. The new release has some support for JDK
...
// ...
// Code for setting up which classes to 
// use as application classes
// ...
...
analyses and anything in between. SPARK is very modular which makes it
...
combinations with other options. Check the Soot mailinglist or visit
...
precision in many analyses and transformations. For example a precise
...
which in turn may lead to more precision in other analyses. Precise
...
usable as a starting point for your own analyses and transformations.
...
performing points-to and null pointer analyses, performing data-flow
