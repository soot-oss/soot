...
\begin{description}
\item[Baf:] a compact rep. of bytecode (stack-based)
\item[Jimple:]  simple, typed,  3-addr (stackless) representation
\item[Shimple:]  SSA-version of Jimple
\item[Grimple:] like Jimple, but with expressions aggregated 
\item[Dava:] structured represenation used for Decompilation 
\end{description}
\end{slide}
...
\end{enumerate}
\item[{\tt java Soot.main --help}]  Lists options.
\item[{\tt java Soot.main A}]     Processes {\tt A.class} in current directory
                        and places produces a new class file in
                        {\tt sootOutput/A.class}.
\item[{\tt java Soot.main -o jimple A}]  Same as above, but produces 
{\tt sootOutput/A.jimple}.
\item[{\tt java Soot.main -O A}] Produces an optimized {\tt A.class}.
\item[{\tt java Soot.main -o dava A}] Decompiles {\tt A.class} and produces
{\tt A.java} in {\tt sootOutput/dava/src/A.java}. 
\item[{\tt java Soot.main -O -app A}]
\end{description}
\end{slide}
...
\item Make some project settings inside Eclipse.
\end{enumerate}
